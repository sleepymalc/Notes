...
\begin{algorithm}[H]\label{algo:find-negative-cyele}
	...
	\KwData{A directed graph \(G = (V, E)\) with edge capacity \(w \colon V \to \mathbb{Z} \)}
	...
	\(\phi \gets \)\Algo{\(G_0^{+M^{\ast} }\), \({w^{\prime}}^{+ M^{\ast}} \)}\Comment*[r]{\hyperref[def:neutralize]{Neutralize} \(G_0^{+M^{\ast} }\)}
