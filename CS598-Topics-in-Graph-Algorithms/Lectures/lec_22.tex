...
\begin{algorithm}[H]\label{algo:Ford-Fulkerson}
	\DontPrintSemicolon{}
	\caption{Ford-Fulkerson Algorithm for \hyperref[prb:s-t-max-flow]{\(s\)-\(t\) Max-Flow}~\cite{ford1956maximal}}
	...
	\BlankLine
	\(f \gets 0\)\Comment*[r]{Initialize a \hyperref[def:flow]{flow}}
	\(c_f \gets c\)\Comment*[r]{Initialize \(G_f\)}
	\While(\label{algo:Ford-Fulkerson-augmenting-path}){\(\exists \text{\(s\)-\(t\) \hyperref[def:augmenting-path]{augmenting path} } P \in G_f \)}{
		\(b \gets \min _{e \in P} c_f(e)\)\Comment*[r]{Compute bottleneck capacity}
		\For(){\(e \in P\)}{
			\(f(e) \gets f(e) + b\)\;
...
\begin{theorem}\label{thm:Ford-Fulkerson-blocking-flow-unit}
...
It's know that \hyperref[def:link-cut-tree]{link/cut trees} can be implemented efficiently~\cite{shiloach1978n,galil1980eviog2v,sleator1981nm,sleator1981data,dominic1983data}.
...
\begin{algorithm}[H]\label{algo:Ford-Fulkerson-link-cut-tree}
	\DontPrintSemicolon{}
	\caption{\hyperref[def:blocking-flow]{Blocking Flow} with \hyperref[def:link-cut-tree]{Link/Cut Tree}}
	\KwData{A connected directed graph \(G = (V, E)\) with edge capacity \(c \colon E \to \mathbb{R} _{+} \), source \(s\), sink \(t\)}
	...
	\While(){\(1 = 1\)}{
		...
		\uIf(\Comment*[f]{Find an \(s\)-\(t\) path to \hyperref[algo:general-adaptive-DFS-block]{block}}){\(r = t\)}{
			...
			\For(){\(e \gets T\).\MinCapacity{\(s\)}}{
				\If(\Comment*[f]{Cut and delete saturated edges}){\(c(e) = 0\)}{
					\(T.\Cut{e}\)\;
					\(E^{\prime} \setminus \{ e \} \)\;
				}
			}
		}\uElseIf(\Comment*[f]{Find an \hyperref[def:admissible]{admissible} edge to \hyperref[algo:general-adaptive-DFS-advance]{advance}}){\(\exists e = (r, v) \in E^{\prime} \)}{
			\(T\).\Link{\(r\), \(v\)}\;
		}\Else(\Comment*[f]{\hyperref[algo:general-adaptive-DFS-retreat]{Retreat} from \(r\)}){
			\If(\Comment*[f]{No more \(s\)-\(t\) path}){\(r = s\)}{
				\(f \gets c - c^{\prime} \)\label{algo:Ford-Fulkerson-link-cut-tree-compute-blocking-flow}\;
				\Return{\(f\)}\;
			}
