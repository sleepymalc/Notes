...
\section{Min-Cuts and Steiner Min-cuts via Tree Packing}
...
Given a directed graph \(G = (V, E)\) with edge capacity \(c \colon E \to \mathbb{R} _{+}\). The \emph{\(s\)-\(t\) max-flow} problem aims to find \(\max _{f} \lvert f \rvert \) where \(f\) ranging over all \hyperref[def:flow]{\(s\)-\(t\) flows} and \(\lvert f \rvert \coloneqq \sum_{e \in \delta ^+(s)} f(e) = \sum_{e \in \delta ^-(t)} f(e)\).
\end{problem}
