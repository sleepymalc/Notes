...
We will need some machinery from combinatorial mathematics, specifically, \hyperref[def:submodular]{submodular} functions.

\begin{definition*}
	Given a finite ground set \(V\), consider a set function \(f \colon 2^V \to \mathbb{R} \).
...
\begin{eg}
	A symmetric \hyperref[def:submodular]{submodular} function is automatically \hyperref[def:posi-modular]{posi-modular}.
\end{eg}

The structural property that we will utilize is the following.

\begin{lemma}\label{lma:union-intersection-min-cut}
	Let \(G = (V, E)\) be a graph and \((A, V\setminus A)\), \((B, V\setminus B)\) be two \hyperref[prb:s-t-min-cut]{\(s\)-\(t\) min-cuts}. Then \(A \cap B\) and \(A \cup B\) are also \hyperref[prb:s-t-min-cut]{\(s\)-\(t\) min-cuts}.
\end{lemma}
\begin{proof}
...
\begin{corollary}\label{col:union-intersection-min-cut}
	For any graph \(G = (V, E)\), there is a unique (inclusion-wise) \hyperref[prb:s-t-min-cut]{\(s\)-\(t\) min-cut}.
\end{corollary}
...
Given a graph \(G = (V, E)\) with edge weight \(c \colon E \to \mathbb{R} \) and a set \(T \subseteq V\) of terminals. The \emph{\(t_i\)-isolating cut} problem aims to find a cut \((U_i, V\setminus U_i)\) such that \(t_i \in U_i\) and \(t_j \notin U_i\) (i.e., \(t_j \in V \setminus U_i\)) for all \(j \neq i\).
\end{problem}
...
The minimum capacity \hyperref[prb:isolating-cut]{\(t_i\)-isolating cut} can be found by a single max-flow computation: by shrinking the terminals in \(T - t_i\) into a single vertex \(s\) and computing the \hyperref[prb:s-t-min-cut]{\(s\)-\(t_i\) min-cut}. Thus, all \hyperref[prb:isolating-cut]{isolating cuts} require \(k\) max-flow computations. The upshot is that this can be done in only \(O(\log k)\) max-flow.

\begin{theorem}\label{thm:isolating-cut}
	Given a graph \(G = (V, E)\) and a terminal set \(T = \{ t_i \} _{i=1}^k\), there is a deterministic algorithm that computes all minimum \hyperref[prb:isolating-cut]{\(t_i\)-isolating cuts} in \(O(\log k)\) max-flow on a graph of size \(\lvert G \rvert \).
\end{theorem}

From \hyperref[def:submodular]{submodularity}, we also have the following, just like \autoref{col:union-intersection-min-cut}.

\begin{lemma}\label{lma:unique-isolating-cut}
	There is a unique minimal \hyperref[prb:isolating-cut]{\(t_i\)-isolating cut} \((U_i, V \setminus U_i)\) such that if \((U, V\setminus U)\) is any \hyperref[prb:isolating-cut]{\(t_i\)-isolating cut}, then \(U_i \subseteq U\).
\end{lemma}

We now describe the algorithm for computing the \hyperref[prb:isolating-cut]{isolating cuts}. Basically, we consider \(h\) bi-partitions \((A_1, B_1), \dots , (A_h, B_h )\) with \(h = \lceil \log k \rceil \), and compute a cut separating each bi-partition. Then, we take the intersection among the resulting cut sets.
...
\begin{algorithm}[H]\label{algo:isolating-cut}
	...
	\KwData{A connected graph \(G = (V, E)\) with edge weight \(c \colon E \to \mathbb{R} \), terminal \(T = \{ t_i \} _{i=1}^{k}\)}
	\KwResult{A set of \hyperref[prb:isolating-cut]{isolating cuts} \(\{ (U_i^{\ast} , V\setminus U_i^{\ast} )\}_{i=1}^{k} \) isolating \(t_i\)'s}
	\SetKwFunction{mincut}{\hyperref[prb:s-t-min-cut]{\(s\)-\(t\)-min-cut}}
	\BlankLine

	\(h \gets \lceil \log k \rceil \)\;
	\For(\label{algo:isolating-cut-for-1}\Comment*[f]{\(h\) bi-partitions \((A_{\ell }, B_{\ell })\) of \(T\)}){\(\ell = 1, \dots , h\)}{
		\(A_{\ell } \gets \{ t_i \mid \text{binary representation of \(i\) has \(1\) in \(\ell^{\text{th}}\) bit} \} \)\;
		\(B_{\ell } \gets T \setminus A_{\ell } \)\;
		\((X_{\ell }, Y_{\ell } ) \gets\)\mincut{\(G\), \(A_{\ell } \), \(B_{\ell } \)}\footnote{This can be done via shrinking \(A_{\ell } \) and \(B_{\ell } \) in to two separate nodes, and compute the \hyperref[prb:s-t-min-cut]{\(s\)-\(t\) min-cut}.}\label{algo:isolating-cut-bi-partition-cut}\Comment*[r]{\(Y_{\ell } = V\setminus X_{\ell } \) }
	}
	\For(){\(i = 1, \dots , k\)}{
	\(U_i \gets \bigcap_{\ell \colon t_i \in A_{\ell } } X_{\ell } \cap \bigcap_{\ell \colon t_i \in B_{\ell } } Y_{\ell } \)\label{algo:isolating-cut-U}\;
	\(H_i = (V_i, E_i)\gets\) shrinking \(V \setminus U_i\) to a single vertex \(s_i\)\label{algo:isolating-cut-H}\;
	\((U_i^{\ast} , V\setminus U_i^{\ast} )\gets\)\mincut{\(H_i\), \(t_i\), \(s_i\)}\label{algo:isolating-cut-min-cut}\;
	}
	\Return{\(\{ (U_i^{\ast} , V\setminus U_i^{\ast} )\}_{i=1}^{k} \)}\;
\end{algorithm}
...
To see the correctness of the \autoref{algo:isolating-cut}, we want to say that \hyperref[prb:s-t-min-cut]{\(s_i\)-\(t_i\) min-cut} is exactly the minimum cost \hyperref[prb:isolating-cut]{\(t_i\)-isolating cut} in \(G\). This is due to \autoref{lma:algo:isolating-cut}.

\begin{lemma}\label{lma:algo:isolating-cut}
	For each \(i\), \((U_i, V\setminus U_i)\) is a \hyperref[prb:isolating-cut]{\(t_i\)-isolating cut}. Furthermore, \(U_i\)'s are pairwise disjoint.
\end{lemma}
\begin{proof}
	Firstly, \(t_i \in A_{\ell } \subseteq X_{\ell }\) or \(t_i \in B_{\ell } \subseteq Y_{\ell } \), implying \(t_i \in U_i\). Consider \(t_j\) with \(j \neq i\). As \(i \neq j\), there is some index \(\ell \) in the binary representation of \(i\) and \(j\) differ in the bit position. Suppose \(i\) has \(1\) in the \(\ell ^{\text{th} }\) position and \(j\) has \(0\), then \(t_i \in A_{\ell } \) and \(t_j \in B_{\ell } \), implying \(t_i \in X_{\ell } \) and \(t_j \notin X_{\ell } \) as \(t_j \in B_{\ell } \subseteq Y_{\ell } \) and \(Y_{\ell } \cap X_{\ell } = \varnothing \). This means \(t_j \notin U_i\).

	We now prove that \(U_i \cap U_j = \varnothing \) for all \(i \neq j\). Firstly, there exists some \(\ell \) such that \(t_i \in A_{\ell } \) and \(t_j \in B_{\ell } \) (or \(t_i \in B_{\ell } \) and \(t_j \in A_{\ell } \)). Suppose \(v \in X_{\ell } \), then \(v\) can't be in \(U_j \subseteq Y_{\ell } \) and if \(v \in Y_{\ell } \), then \(v\) can't be in \(U_i \subseteq X_{\ell } \), hence \(v\) can't be in both \(U_i\) and \(U_j\).
\end{proof}

\autoref{lma:algo:isolating-cut} gives the following picture, where each \(t_i\) lives in exactly one \(U_i\).
...
Hence, for each \(i\), we have a \hyperref[prb:isolating-cut]{\(t_i\)-isolating cut} \((U_i, V\setminus U_i)\). \autoref{lma:unique-isolating-cut} states that there is a \hyperref[prb:isolating-cut]{\(t_i\)-isolating min-cut} \((U_i^{\ast} , V\setminus U_i^{\ast} )\) where \(U_i^{\ast} \subseteq U_i\). More explicitly, consider the following:

\begin{lemma}\label{lma:unique-isolating-min-cut}
	There exists a unique minimal \hyperref[prb:isolating-cut]{\(t_i\)-isolating min-cut} \((U_i^{\ast} , V \setminus U_i^{\ast} )\) such that \(U_i^{\ast} \subseteq U_i\).
\end{lemma}
\begin{proof}
	It suffices to prove that if \(t_i \in A_{\ell } \) then \(U_i^{\ast} \subseteq X_{\ell } \). Assume not, then \(U_i^{\ast} \cap (V\setminus X_{\ell } ) \neq \varnothing \). But since \(U_i^{\ast} \cap X_{\ell } \) is a \hyperref[prb:isolating-cut]{\(t_i\)-isolating cut}, while \(U_i^{\ast} \) is the minimal \hyperref[prb:isolating-cut]{\(t_i\)-isolating min-cut}, \(\lvert \delta (U_i^{\ast} \cap X_{\ell } ) \rvert > \lvert \delta (U_i^{\ast} ) \rvert \).

	Moreover, it's trivial to see that \(U_i^{\ast} \cup X_{\ell } \) is a \(A_{\ell } \)-\(B_{\ell } \) cut (not necessarily minimum, just a cut), hence we also have \(\lvert \delta (U_i^{\ast} \cup X_{\ell } ) \rvert \geq \lvert \delta (X_{\ell } ) \rvert \). From \hyperref[def:submodular]{submodularity} of \(\lvert \delta (\cdot) \rvert \), we have
	\[
		\lvert \delta (U_i^{\ast} ) \rvert + \lvert \delta (X_{\ell } ) \rvert
		\geq \lvert \delta (U_i^{\ast} \cap X_{\ell } ) \rvert + \lvert \delta (U_i^{\ast} \cup X_{\ell } ) \rvert,
	\]
	which is a contradiction.
\end{proof}

With all the lemmas, it's now easy to see that \autoref{algo:isolating-cut} is at least correct. Firstly, from \autoref{lma:unique-isolating-min-cut}, we know that there the optimal \hyperref[prb:isolating-cut]{\(t_i\)-isolating min-cut} \(U_i^{\ast} \subseteq U_i\) (here, \(U_i^{\ast} \) is not necessary the one found by \autoref{algo:isolating-cut}: indeed, we're trying to argue this). As \(U_i\)'s are disjoint, each terminal \(t_i\) lives in exactly one \(U_i\), hence computing \hyperref[prb:s-t-min-cut]{\(s_i\)-\(t_i\) min-cut} will indeed recover \(U_i^{\ast} \).

\begin{intuition}
	When we contract \(V \setminus U_i\), we do not lose the optimal \hyperref[prb:isolating-cut]{isolating cut} \(U_i^{\ast} \).
\end{intuition}
...
\end{theorem}
\begin{proof}
	We analyze the runtime. It's easy to see that \autoref{algo:isolating-cut-for-1} requires \(O(\log k)\) max-flow computations on \(G\). It's also easy to show that computing \(U_i\)'s in \autoref{algo:isolating-cut-U} can be done in \(O((m+n) \log k)\) time given \((X_{\ell } , Y_{\ell } )\) for \(\ell \in [h]\). However, \autoref{algo:isolating-cut-H} and \autoref{algo:isolating-cut-min-cut} seem to require \(k\) max-flow computations.

	\begin{claim}
		In total, \autoref{algo:isolating-cut-min-cut} only requires \(O(\log k)\) max-flow computations.
	\end{claim}
	\begin{explanation}
		Let us understand the size of \(H_i\). It has \(n_i + 1\) vertices where \(n_i = \lvert U_i \rvert \), and it has \(m_i\) edges where \(m_i = \lvert E(U_i) \rvert + \lvert \delta (U_i) \rvert \). Thus, the running time of max-flow on \(H_i\) is \(T(n_i + 1, m_i)\) where \(T(a, b)\) is the running time of max-flow on graph with \(a\) nodes and \(b\) edges. We observe that \(\sum_{i} (n_i + 1) \leq 2n\) since \(U_i\)'s are disjoint, while \(\sum_{i} m_i \leq 2m\): consider any edge \(uv \in E\). If \(uv \in E(U_i)\) for some \(i\), then it does not contribute to any other \(H_j\). If \(uv \in \delta (U_i)\) for some \(i\), then it can be in \(\delta (U_j)\) for only one more index \(j \neq i\).
...
\begin{algorithm}[H]\label{algo:Steiner-min-cut}
	...
	\KwData{A connected graph \(G = (V, E)\) with edge weight \(c \colon E \to \mathbb{R} \), terminal \(T = \{ t_i \} _{i=1}^{k}\)}
	\KwResult{A possible \hyperref[prb:Steiner-min-cut]{Steiner min-cut} \((U^{\ast} , V\setminus U^{\ast} )\)}
	\SetKwFunction{sample}{Sample}
	\SetKwFunction{isolatingcut}{\hyperref[algo:isolating-cut]{Isolating-Cut}}
	\SetKwFunction{mincost}{Min-Cost}
	\BlankLine

	\(U^{\ast} \gets \varnothing \)\Comment*[r]{Initialize \hyperref[prb:Steiner-min-cut]{Steiner min-cut}}
	\For(){\(i = 0, \dots , \lceil \log k \rceil \)}{
	\(T^{\prime} \gets\)\sample{\(T\), \(1 / 2^i\)}\Comment*[r]{Sample each terminal in \(T\) with probability \(1 / 2^i\)}
	\(\{ (U_i^{\ast} , V\setminus U_i^{\ast} ) \}_{i=1}^{\lvert T^{\prime} \rvert } \gets\)\isolatingcut{\(G\), \(c\), \(T^{\prime} \)}\;
	\(U^{\ast} \gets \)\mincost{\(\{ (U_i^{\ast} , V\setminus U_i^{\ast}) \}_{i=1}^{\lvert T^{\prime} \rvert } \cup \{ (U^{\ast} , V\setminus U^{\ast} ) \} \)}\Comment*[r]{Update minimum cost cut}
	}
	\Return{\((U^{\ast} , V\setminus U^{\ast} )\)}\;
\end{algorithm}
