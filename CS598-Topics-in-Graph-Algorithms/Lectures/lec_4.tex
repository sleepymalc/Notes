...
\begin{algorithm}[H]\label{algo:isolating-cut}
	...
	\((S_i^{\ast} , V\setminus S_i^{\ast} )\gets\)\mincut{\(H_i\), \(t_i\), \(s_i\)}\label{algo:isolating-cut-min-cut}\;
	}
...
\begin{algorithm}[H]\label{algo:Steiner-min-cut}
	...
	\(U^{\ast} \gets \)\mincost{\(\{ (S_i^{\ast} , V\setminus S_i^{\ast}) \}_{i=1}^{\lvert T^{\prime} \rvert } \cup \{ (U^{\ast} , V\setminus U^{\ast} ) \} \)}\Comment*[r]{Update minimum cost cut}
	}
