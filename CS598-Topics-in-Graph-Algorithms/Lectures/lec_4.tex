...
\begin{theorem}
	There is a randomized algorithm that given \(G=(V, E)\) and terminal set \(T \subseteq V\) of size \(k\), outputs the \hyperref[prb:Steiner-min-cut]{Steiner min-cut} with high probability using \(O(\log^2 k \log n)\) max-flow computations on graphs with \(\lvert V \rvert \) vertices and \(\lvert E \rvert \) edges.
\end{theorem}
\begin{proof}
