...
\[
	\begin{split}
		X_0 &\sim \mathrm{uniform}(\{1, 2, \ldots , \left\vert \mathcal{V} \right\vert\})\\
		X_1 &=\begin{dcases}
			\mathrm{uniform}(\{1, 2, \ldots , \left\vert \mathcal{V} \right\vert \}), & \text{ with probability }1 - s; \\
...
\[
	\mathop{\mathrm{Var}}\left[\hat{\pi}_v(s) \right]  = \frac{1}{\left\vert \mathcal{V}  \right\vert M}\left(\pi_{j} - \frac{\sum_{i=1}^{\left\vert \mathcal{V}  \right\vert} \sigma_{ij}^2}{\left\vert \mathcal{V}  \right\vert}\right)
\]
