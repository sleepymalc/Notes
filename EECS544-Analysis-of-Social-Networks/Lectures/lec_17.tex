...
\begin{note}[Formal Analysis]
	A more formal setup is the following. Let \(\left\{X_j(t)\right\}_{j = 1, \ldots , V}\) be a \hyperref[def:Markov-chain]{Markov chain}. Considering two aggregations:
	\begin{enumerate}
		\item For \(i = 0, 1, \ldots \), we determine the fraction of nodes with \hyperref[def:in-degree]{in-degree} at least \(i\) at time \(t\). Then
		      ...
		      \]
		\item For \(s\in[0, 1]\), define \(t = \left\lfloor sV\right\rfloor \). Now, for \(i = 0, 1, \ldots \), set \(\widetilde{y}^V_t(s) = y_{i}(\left\lfloor sV\right\rfloor )\). Then the formal comparison of \(\widetilde{y}^V_i(s)\) can be made with an appropriate O.D.E. solution \(\left\{y_{i}(s)\right\}_{i = 0, 1, \ldots }\) for \(s\in [0, 1]\). This is the formal \href{https://en.wikipedia.org/wiki/Mean-field_theory}{mean-field} analysis that is not in the scope of the course. Hence, we give a flavor of the results using a hand-wavy argument.
	\end{enumerate}
\end{note}