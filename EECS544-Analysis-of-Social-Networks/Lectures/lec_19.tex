...
\begin{prev}
	...
	\begin{itemize}
		...
		\item \hyperref[def:reward]{Utilities}: \(\forall i\in \mathcal{I} \),
		      ...
		      \]
		      For every vector \(s = (s_1, s_2, \ldots , s_I)\in \prod\limits_{j\in \mathcal{I}} \mathcal{S}_j\), \(u_{i}(s)\) is a real number.
	\end{itemize}
	\begin{remark}
...
\begin{prev}[Nash equilibrium]
	Let \(s^{\ast} = (s_1^{\ast}, s_2^{\ast}, \ldots , s_I^{\ast})\) be the \textbf{strategy profile}, which is a \hyperref[def:pure-strategy]{pure strategy} \hyperref[def:Nash-equilibrium]{Nash equilibrium} if
	\[
