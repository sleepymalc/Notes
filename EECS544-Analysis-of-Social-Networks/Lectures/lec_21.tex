...
\begin{remark}
	...
	\]
	for every \(t_{i}\), \(\mathrm{BR}_{i}(\sigma_{-i}) = \overline{\sigma}_{i}(\sigma_{-i})\). Then we call that \((\sigma^{\ast}_1, \dots , \sigma^{\ast}_I)\) is a \hyperref[def:mathematical-Bayesian-game]{Bayes} \hyperref[def:interim]{interim} \hyperref[def:Nash-equilibrium]{Nash equilibrium} if
	\[
