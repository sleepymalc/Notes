...
\chapter{Page Rank}
In this chapter, we still try to analyze a given network, but now we're focusing on nodes-importance rather than links-importance. Our goal is
to develop the so-called \textbf{page rank}, which is inspired by the following algorithm.

\section{HITS Algorithm}
\hyperref[algo:HITS-algorithm]{HITS} is a hyperlinks-induced topic search algorithm proposed by \href{https://en.wikipedia.org/wiki/Jon_Kleinberg}{John Kleinberg},
whose backbones are the idea of \textbf{hubs} and \textbf{authorities} and the \textbf{voting mechanism}.
\begin{itemize}
...
We first see the pseudocode of the \hyperref[algo:HITS-algorithm]{algorithm}.
