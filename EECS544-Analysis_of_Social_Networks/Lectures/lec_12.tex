...
\]
where \(\rho(A)\) is the unique \hyperref[def:spectral-radius]{spectral radius}, \(v\) is the right eigenvector of
\(A\) and \(w\) is the left eigenvector of \(A\), and \(w^{\top}v = 1\) for \(\rho(A)\).
...
\begin{remark}
	...
	\begin{itemize}
		...
		\item \(r^{\ast} = A r^{\ast}\) need not hold because \(1\) need not be an eigenvalue.
		\item Since
		      \[
...
\]
which implies \(\alpha\) should not be the inverse of an eigenvalue of \(A\). Also, we  need that \(r^{\ast}\) to be positive.
...
Then for all \(1/\alpha > \rho(A) \iff \alpha\rho(A)<1 \iff \rho(\alpha A)<1\), \(\alpha\) can't be the inverse of any eigenvalues of \(A\) since
...
\chapter{Scaled Page Rank via Markov Chain}
\begin{problem}
...
\begin{intuition}[Stochastic process]
