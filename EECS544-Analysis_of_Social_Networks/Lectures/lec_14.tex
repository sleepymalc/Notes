...
\begin{answer}
	...
	\begin{itemize}
		...
		\item Reset
	\end{itemize}
	where we restart/reset to a specific node \(v^{\ast}\in \mathcal{V}\).
	\begin{remark}
...
\begin{definition}
	...
	\]
\end{definition}
Collect \(\pi_{v^{\ast}}(s)\) for all \(v^{\ast}\in\mathcal{V}\), then if we reset uniformly,
\[
	\pi(s) = \frac{1}{V}\sum\limits_{v^{\ast}\in\mathcal{V}} \pi_{v^{\ast}}(s).
\]
