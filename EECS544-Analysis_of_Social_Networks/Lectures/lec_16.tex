...
Now, let \(V\) denotes the number of nodes of the \hyperref[def:graph]{graph} we're going to generate,
...
\begin{algorithm}[H]\label{algo:uniform-and-undirected-model-algo}
	...
	\For(){\(i= 1, \ldots  , V-1\)}{
		\For(){\(j=i+1, \ldots  , V\)}{
			\(A_{ij} \gets \)\bern{\(p\)}\Comment*[r]{\(\probability{}{A_{ij} = 1} = p\), \(\probability{}{A_{ij} = 0} = 1-p\)}
		}
	}
	\Return{\(A\)}\;
\end{algorithm}
...
\begin{algorithm}[H]\label{algo:non-uniform-and-undirected-model-algo}
	...
	\For(){\(i= 1, \ldots  , V-1\)}{
	\For(){\(j=i+1, \ldots  , V\)}{
	\(A_{ij} \gets \)\bern{\(P_{ij}\)}\Comment*[r]{\(\probability{}{A_{ij} = 1} = P_{ij}\), \(\probability{}{A_{ij} = 0} = 1-P_{ij}\)}
	}
	}
	\Return{\(A\)}\;
\end{algorithm}
