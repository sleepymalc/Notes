...
Given a finite \hyperref[def:set-system]{set system} \((U, \mathcal{\MakeUppercase{S}} )\) where \(\mathcal{\MakeUppercase{S}} \coloneqq \left\{ S_i\subseteq U\right\}_{i=1}^m \), find a \hyperref[def:covering]{covering} \(\mathcal{\MakeUppercase{S}} ^\prime \) while minimizing \(\left\vert \mathcal{\MakeUppercase{S}} ^\prime \right\vert\).
\end{problem}
...
\subsection{Greedy: In terms of \(k\)}
We first see the algorithm.

\begin{algorithm}[H]\label{algo:set-cover-greedy-I}
	\DontPrintSemicolon
	...
	\While(\Comment*[f]{\(O(n)\)}){\(U \neq \varnothing \)}{
		...
		\(U\gets U\setminus S_i\)\;\label{line5:algo:set-cover-greedy-I}
		\For(){\(e\in U \cap S_i\) }{
			\(y_e \gets 1 / \left\vert U \cap S_i \right\vert \)\Comment*[r]{Average costs}
		}
		\(i\gets i+1\)\;
	}
...
\begin{remark}
	It's clear that \autoref{algo:set-cover-greedy-I} is a polynomial time algorithm, also, the output \(\mathcal{\MakeUppercase{S}} ^\prime \) is always a valid \hyperref[def:covering]{covering}.
\end{remark}

\begin{theorem}
	\autoref{algo:set-cover-greedy-I} is an \(H_k\)-approximation\footnote{\(H_k\) is the so-called \emph{harmonic number}, which is defined as \(\sum_{i=1}^{k} 1 / i \leq \ln k + 1\).} algorithm.
\end{theorem}
\begin{proof}
	Denote the \(\OPT\) as \(\mathcal{\MakeUppercase{S}} ^{\ast} \coloneqq \left\{ S_1^{\ast} , \ldots , S_{\ell}^{\ast} \right\}\), and first note that the average cost \(y_e\) essentially maintains \(\sum_{e\in U} y_e = \left\vert \mathcal{\MakeUppercase{S}} ^\prime  \right\vert\), hence we just need to bound \(y_e\) w.r.t. \(S^{\ast}\). To do this, for any \(S^{\ast} \in \mathcal{\MakeUppercase{S}} ^{\ast} \), say \(S_1^{\ast} \eqqcolon \left\{ e_1, \ldots , e_k  \right\} \) where we number \(e_i\) in terms of the order of which being deleted, i.e., \(e_1\) is deleted first from \(U\) (\autoref{line5:algo:set-cover-greedy-I}), etc.
	...
	\end{claim}
	\begin{explanation}
		Consider the iteration when \(e_i\) was picked by \(S^\prime\), i.e., \(\left\vert U \cap S^\prime  \right\vert \geq \left\vert U \cap S_1 ^{\ast} \right\vert \geq k - i + 1\), then by definition (\autoref{line5:algo:set-cover-greedy-I}) we have \(y_{e_i} = \frac{1}{\vert U \cap S^\prime  \vert } \leq \frac{1}{\vert U \cap  S_1 ^{\ast} \vert} \leq \frac{1}{k - i + 1}\).
	\end{explanation}

	We immediately see that whenever the optimal solution pays \(1\) (for choosing \(S_1^{\ast}\) for instance), \autoref{algo:set-cover-greedy-I} pays at most \(H_k\) since \(\sum_{e_i\in S_1^{\ast} }y_{e_i} \leq \sum_{i=1}^{k} \frac{1}{k-i+1} = H_k\), or more formally,
	\[
		\left\vert \mathcal{\MakeUppercase{S}} ^\prime  \right\vert = \sum_{e\in U}y_e \leq \sum_{S_i^{\ast} \in \mathcal{\MakeUppercase{S}} ^{\ast} }\underbrace{\sum_{e\in S_i^{\ast} }y_e}_{\leq H_k} \leq \ell \cdot H_k = H_k\cdot \OPT ,
	\]
	which finishes the proof.
\end{proof}

In all, observe that \(H_k \leq \ln k + 1\), we see that \autoref{algo:set-cover-greedy-I} is a \((\ln k)\)-approximation algorithm.