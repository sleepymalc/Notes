...
\begin{equation}\label{eq:lec10-1}
	\begin{alignedat}{3}
		...
		& \sum_{e\in E(S, \overline{S} )} x_e \leq \left\vert S \right\vert - 1 & \forall S \subseteq \mathcal{\MakeUppercase{v}} \\
		& x \geq 0.
	\end{alignedat}
\end{equation}
...
\begin{algorithm}[H]\label{algo:min-spanning-tree-pipage-rounding}
	...
	\KwData{A connected graph \(\mathcal{\MakeUppercase{g}} = (\mathcal{\MakeUppercase{v}} , \mathcal{\MakeUppercase{e}} )\), a weight function \(w\colon \mathcal{\MakeUppercase{e}} \to \mathbb{\MakeUppercase{r}} ^+\)}
	\KwResult{A minimum \hyperref[def:spanning-tree]{spanning tree} \(T\)}
	\SetKw{Break}{break}
	\SetKwFunction{Swap}{swap}
	\SetKwFunction{LP}{LP-Solver}
	\SetKwFunction{SG}{Subgraph}
	\BlankLine
	\(x\gets\)\LP{\autoref{eq:lec10-1}}\Comment*[r]{with \hyperref[rmk:separation-oracle]{separation oracle}}
	\;
	\label{algo:min-spanning-tree-pipage-rounding-rounding}\Comment{Pipage Rounding}
	\While(\Comment*[f]{not integral}){\(x \notin \mathbb{\MakeUppercase{n}} ^{m}\)}{
		...
		\While(\label{algo:min-spanning-tree-pipage-rounding-increase}\Comment*[f]{by solving \autoref{eq:lec10-2}}){increase \(x_f\) and decrease \(x_g\) by a unit}{
		...
		\item \autoref{algo:min-spanning-tree-pipage-rounding-increase}: This continuous process is done by taking \(\delta \) from solving the following LP as the total unit we should increase/decrease:
		      \begin{equation}\label{eq:lec10-2}
			      \begin{alignedat}{4}
				      ...
				      & \sum_{e\in E(S)} y- e \leq \left\vert S \right\vert - 1 &\quad \forall S \subseteq \mathcal{\MakeUppercase{v}} \\
				      & 0 \leq y \leq 1,
			      \end{alignedat}
		      \end{equation}
...
Our goal now is to show that during the \hyperref[algo:min-spanning-tree-pipage-rounding-rounding]{pipage rounding}:
\begin{enumerate}[(a)]
	\item \(x\) remains feasible;
	\item \(\sum_{e} x_e w(e)\) remains unchanged (does not increase).
\end{enumerate}
...
\begin{algorithm}[H]\label{algo:min-spanning-tree-randomized-pipage-rounding}
	...
	\While(\Comment*[f]{not integral}){\(x \notin \mathbb{\MakeUppercase{n}} ^{m}\)}{
		...
		\(a\gets \max_a x_f \gets x_f + a, x_g \gets x_g - a\) feasible\Comment*[r]{\(a > 0\)}
		\(b\gets \max_b x_f \gets x_f - b, x_g \gets x_g + b\) feasible\Comment*[r]{\(b > 0\)}
