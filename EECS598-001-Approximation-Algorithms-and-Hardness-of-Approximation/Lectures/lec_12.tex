...
\begin{algorithm}[H]\label{algo:random-thin-spanning-tree}
	...
	\KwData{A connected graph \(\mathcal{\MakeUppercase{g}} = (\mathcal{\MakeUppercase{v}} , \mathcal{\MakeUppercase{e}} )\), weight \(w\colon \mathcal{\MakeUppercase{e}} \to \mathbb{\MakeUppercase{r}} ^+\), solution \(z\) of \autoref{eq:spanning-tree-polytope}}
...
\begin{proof}
	...
	\[
		\begin{split}
			...
			&\leq \sum_{i=1}^{\infty} (2n)^{2(i+1)} \cdot \left( \frac{e}{\beta } \right) ^{\beta \cdot i\cdot \lambda}\\
...
\begin{algorithm}[H]\label{algo:random-construct-tour}
	...
	\BlankLine
	\(T^\prime\gets\)\TSP{\(\mathcal{\MakeUppercase{g}} \), \(w\), \(z\)}\;
