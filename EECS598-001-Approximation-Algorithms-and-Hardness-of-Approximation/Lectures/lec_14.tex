...
Let \([n] \coloneqq \mathcal{V} \) given that \(n \coloneqq \left\vert \mathcal{V}  \right\vert \), and denote the variables being \(x_i\) for \(i = 1, \ldots  , n\) be \(1\) if \(i\in S\), \(-1\) if \(i \notin S\). Then the following programming captures \hyperref[prb:max-cut]{max cut} when optimizing over \(x_i\in\mathbb{R} \):
\[
	\begin{aligned}
