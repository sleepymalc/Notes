...
In this chapter, we'll talk about the constraint satisfaction problem and solve it by \hyperref[def:SDP]{semidefinite programming} specifically. Toward doing this, we first develop some useful tools related to this.

\section{Positive Semidefinite Matrix}
...
To solve this, we relax \(x_i\in \mathbb{\MakeUppercase{r}} \) to \(u_i\in \mathbb{\MakeUppercase{r}} ^n\), we have
\[
	\begin{aligned}
		\max~ & \sum_{(i, j)\in \mathcal{\MakeUppercase{e}}} ( 1 - \left\langle u_i, u_j \right\rangle ) / 2 \\
		      & \left\lVert u_i\right\rVert _2 ^{2} = 1.
	\end{aligned}
\]
...
\begin{equation}\label{eq:max-cut}
	\begin{aligned}
		\max~ & \sum_{(i, j)\in \mathcal{\MakeUppercase{e}}} ( 1 - X_{ij} ) / 2 \\
		      & X_{ii} = 1                                                      \\
		      & X \succeq 0
	\end{aligned}
\end{equation}
...
\begin{algorithm}[H]\label{algo:max-cut-randomized-rounding}
	...
	\KwData{A connected graph \(\mathcal{\MakeUppercase{g}} = (\mathcal{\MakeUppercase{v}} , \mathcal{\MakeUppercase{e}} )\), a solution \(X\) for \autoref{eq:max-cut}}
	\KwResult{A cut \(S \subseteq \mathcal{\MakeUppercase{v}} \)}
	...
	\(S\gets \left\{ i\in \mathcal{\MakeUppercase{v}} \colon \left\langle g, u_i \right\rangle \geq 0 \right\} \)\Comment*[r]{Choose one side}
	\Return{\(S\)}\;
\end{algorithm}

We see that given \((i, j)\in \mathcal{\MakeUppercase{e}} \), denote the contribution to the \hyperref[eq:max-cut]{SDP} as \(S_{ij} \), and we have
\[
...
\begin{notation}[Goemans-Williamson constant]
...
\begin{theorem}[\cite{goemans1995improved}]
