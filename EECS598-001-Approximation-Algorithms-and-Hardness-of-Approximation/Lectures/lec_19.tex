...
The proof is omitted since it worth a complete course dedicated on which. But to understand \href{thm:PCP}, we need to understand the class \hyperref[def:PCP]{PCP}. First, recall the definition of \(\NP\).
...
To decipher \autoref{def:PCP}, a randomized Turing machine \(V\) decides both the position (\(Q_1, \ldots , Q_q \)) are allowed to access, and also a function \(\phi _R\) which only looks at \(x_{Q_1}, x_{Q_2}, \ldots  , x_{Q_q}\), acting as a decider for \(V\). With this characterization, \autoref{thm:PCP} is equivalent as saying that there exists \(\epsilon > 0\) such that
\[
