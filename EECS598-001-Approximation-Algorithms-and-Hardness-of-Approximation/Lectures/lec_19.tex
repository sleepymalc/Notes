...
\begin{remark}
	The term \hyperref[def:completeness]{completeness} and \hyperref[def:soundness]{soundness} comes from logic, where more intuitively, for a proof system, \hyperref[def:completeness]{completeness} states that every true statement has a proof, while \hyperref[def:soundness]{soundness} states that every false statement can't have a proof, i.e., we can't prove anything that is wrong.
\end{remark}
