...
\begin{proof}
	...
	which implies
	\[
		\frac{\OPT}{d}\leq \OPT_\text{LP} \leq \OPT.
	\]
	\begin{note}
...
\begin{proof}
	...
	\[
		\Pr(\overline{\text{\(e\) is covered}}) = \prod\limits_{i=1}^{d} (1 - x_i)\leq \prod\limits_{i=1}^{d} e^{- x_i} = e^{- (x_1 + \ldots x_d)} \leq e^{-1}.
	\]
\end{proof}

A standard way to boost the correctness of a randomized algorithm is just to run it multiple time. Hence, we have hte following.
...
\begin{theorem}
	With \(\alpha = 2\ln n\), \(\mathcal{\MakeUppercase{S}} ^\prime\) is a \hyperref[def:covering]{covering} w.p. at least \(1 - \frac{1}{n}\).
\end{theorem}
\begin{proof}
	...
	\]
	This implies w.p. \(\geq 1 - 1/n\), \(\mathcal{\MakeUppercase{S}} ^\prime \) is a \hyperref[def:covering]{covering}.
\end{proof}

\begin{theorem}
	With \(\alpha = 2 \ln n\), \autoref{algo:set-cover-randomized-LP-rounding-multi-time} is a -approximation algorithm w.p. at least
\end{theorem}
\begin{proof}
	Since \(\expectation{}{w(\mathcal{\MakeUppercase{S}} ^\prime )} \leq \alpha \sum_{i}w_{i} x_{i} = \alpha \OPT_{\text{LP} }\), we have
	\[
		\Pr(w(\mathcal{\MakeUppercase{S}} ^\prime )\geq 2\cdot \alpha \OPT_{\text{LP} })\leq \frac{1}{2}
	\]
	from Markov inequality. Now, we see that w.p. \(\geq 1 / 2\),
	\[
