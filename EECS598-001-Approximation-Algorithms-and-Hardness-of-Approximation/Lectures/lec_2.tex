...
\begin{lemma}\label{lma:lec2-3}
	With \(\alpha = 2 \ln n\), \(\mathcal{\MakeUppercase{S}} ^\prime \) returned from \autoref{algo:set-cover-randomized-LP-rounding-multi-time} has an approximation ratio \((4 \ln n)\).\footnote{Note that \(\mathcal{\MakeUppercase{S}} ^\prime \) is not necessary a \hyperref[def:covering]{covering}.}
\end{lemma}
\begin{proof}
	Since \(\expectation{}{w(\mathcal{\MakeUppercase{S}} ^\prime )} \leq \alpha \sum_{i}w_{i} x_{i} = \alpha \OPT_{\text{LP} }\), we have \(\Pr(w(\mathcal{\MakeUppercase{S}} ^\prime )\geq 2\cdot \alpha \OPT_{\text{LP} })\leq 1 / 2\) from Markov inequality. Now, we see that w.p. \(\geq 1 / 2\),
	\[
