...
Just like \hyperref[prb:max-3LIN]{3LIN}, we design a \hyperref[not:dictation]{dictator-ship} test, namely to create an instance (a graph) \(\mathcal{G} =(\mathcal{V} , \mathcal{E} )\) for \(\mathcal{V} \coloneqq \left\{ \pm 1 \right\} ^R\) such that
\begin{itemize}
