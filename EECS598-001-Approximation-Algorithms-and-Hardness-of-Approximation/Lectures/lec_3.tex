...
We now give another LP formulation for \hyperref[prb:set-cover]{set cover}. Given \(\mathcal{\MakeUppercase{S}} = \left\{ S_1, \ldots , S_m  \right\} \), \(U = \left\{ e_1, \ldots , e_n  \right\} \) and define \(A\in \mathbb{\MakeUppercase{r}} ^{n\times m}\) such that
\[
	A_{ij} = \begin{dcases}
		1, & \text{ if } e_i\in S_j ; \\
		0, & \text{ otherwise}.
...
Now, recall the \hyperref[clm:set-cover-cost]{claim} in \autoref{thm:set-cover-k-approx}, i.e., \(y_{i} \leq \frac{w_{j} }{k - i + 1}\). We see that the \(y_{i} \) are just the dual variables in our setup. Additionally, we have the following lemma.
\begin{lemma}\label{lma:lec3-1}
	\(y/H_k\) is dual-feasible for \((D)\).
\end{lemma}
\begin{proof}
	We see that \(y_{i} \geq 0\) trivially, so we only need to show that \(\sum_{i=1}^{n} A_{ij}y_{i} \leq w_{j}\) for \(j\in [n]\). But this is trivial since given a set \(S_j\in \mathcal{\MakeUppercase{S}} \), for every \(e_i\in S_j\) for \(i = 1, \ldots , k \), we pay at most \(w_j / i\) for them, hence we're done.
\end{proof}
...
\begin{remark}[Dual fitting]\label{rmk:dual-fitting}
	The above method is called \emph{dual fitting}. The way to do this is the following.
	\begin{enumerate}
