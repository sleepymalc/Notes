...
\begin{remark}[Reversed greedy]\label{rmk:reversed-greedy}
	In \autoref{algo:feedback-vertex-set-PD}, the method we turn \(F\) into its minimal is called \emph{reversed greedy}. This is just checking that if we remove a vertex \(v\) from \(F^\prime \) while \(F^\prime \) is still feasible, then we just do it. Additionally, we iterate through \(v\) in the \textbf{reversed} order w.r.t. how \(v\) is being added into.
\end{remark}
