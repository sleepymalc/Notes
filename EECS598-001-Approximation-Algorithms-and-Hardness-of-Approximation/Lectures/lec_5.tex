...
\begin{algorithm}[H]\label{algo:facility-location-PD}
	...
	\KwData{A set of clients \(P\subseteq X\), a set of (possible) facilities \(Q\subseteq X\)}
	...
	\uIf(\label{algo:facility-location-PD-if-1}\Comment*[f]{\(j\) gets tight (open)}){some \(j\in Q\setminus O\) s.t. \(\sum_{i} \beta _{ij} = f\)}{
		\Break
	}
	...
	\(O\gets\left\{ \text{tight facilities}\right\} \)\Comment*[r]{Update \(O\)}
	\(S\gets \left\{ \text{clients connected to \(O\)} \right\} \)\Comment*[r]{Update \(S\)}
	}
