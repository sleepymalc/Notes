...
\begin{algorithm}[H]\label{algo:facility-location-PD}
	...
	\uIf(\label{algo:facility-location-PD-if-1}\Comment*[f]{\(j\) gets tight (open)}){some \(j\in Q\setminus O\) s.t. \(\sum_{i\in P} \beta _{ij} = f\)}{
		\Break
	}
	\ElseIf(\label{algo:facility-location-PD-if-2}\Comment*[f]{\(i\) can connect to \(j\in O\)}){some \(i\in P\setminus S\) s.t. \(\alpha_i \geq d(i, j)\)}{
		\Break
	}
	}
	\(O\gets\left\{ \text{tight facilities}\right\} \)\label{algo:facility-location-PD-lin10}\Comment*[r]{Update \(O\)}
	\(S\gets \left\{ \text{clients connected to \(O\)} \right\} \)\label{algo:facility-location-PD-lin11}\Comment*[r]{Update \(S\)}
	}

	\;
	\label{algo:facility-location-PD-comment}\Comment{Trim down \(O\)}
	\(G = (O, E\coloneqq \left\{ (j, j^\prime )\colon \exists i\in P \text{ such that }\alpha _i > d(i, j), j, j^\prime \in O \right\})\)\;
	Compute \(O^\prime\) s.t. \(\forall j\in O\), either \(j\in O^\prime\) or \(\exists j^\prime\in O^\prime \) s.t. \((j, j^\prime )\in E\)\Comment*[r]{\hyperref[prb:max-strongly-independent-set]{maximal independent set}}
	\Return{\(O^\prime \)}\;
\end{algorithm}
