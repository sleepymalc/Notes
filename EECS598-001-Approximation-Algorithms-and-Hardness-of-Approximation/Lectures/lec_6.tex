...
\begin{remark}[Hardness in temrs of LMP]
	For \autoref{prb:facility-location}, we have the following.
	\begin{enumerate}[(a)]
...
\begin{algorithm}[H]\label{algo:facility-location-greedy}
	...
	\KwData{A set of clients \(P\subseteq X\), a set of (possible) facilities \(Q\subseteq X\), facility cost \(f\)\footnote{We didn't use it explicitly in the alogirhtm since we hide it in the cost function \(c(\cdot)\).}}
	\KwResult{A set of opened facilities \(O\subseteq Q\)}
	\SetKw{Break}{break}
	\BlankLine
...
\begin{algorithm}[H]\label{algo:facility-location-greedy-I}
	...
	\uIf(\Comment*[f]{\(j\) gets tight (open)}){some \(j\in Q\setminus O\) s.t. \(\sum_{i\in P\mathcolor{red}{\setminus S}} \beta _{ij} = f\)}{
		\Break
	}
	\ElseIf(\Comment*[f]{\(i\) can connect to \(j\in O\)}){some \(i\in P\setminus S\) s.t. \(\alpha_i \geq d(i, j)\)}{
		\Break
	}
	}
	\(O\gets \mathcolor{red}{O \cup \left\{ j \right\}}\)\Comment*[r]{Update \(O\)}
...
Another potential modification gives us a \(1.61\)-approximation. We essentially allow \(i\in S\) to \emph{switch} in \autoref{algo:facility-location-greedy-I}, i.e., after \(i\) connects to \(j\), if \(j^\prime \) is closer to \(i\) later, \(i\) can \emph{offer} \(d(i, j) - d(i, j^\prime )\).
...
\begin{algorithm}[H]\label{algo:facility-location-greedy-II}
	...
	\If(){some \(j\in Q\setminus O\) s.t. \(\mathcolor{red}{\sum_{i\in S} (d(i, w(i)) - d(i, j))^+ } + \sum_{i\in P\setminus S} \beta _{ij} = f\)\footnote{We define \(a^+ \coloneqq \max (a, 0)\) and also, \(w(i)\) is now the \emph{current} facility \(i\) is connected to.}}{
		...
		\If(\Comment*[f]{\(i\) can connect to \(j\in O\)}){some \(i\in P\setminus S\) s.t. \(\alpha_i \geq d(i, j)\)}{
			\Break
		}
	}
	\(O\gets O \cup \left\{ j \right\} \)\Comment*[r]{Update \(O\)}
...
\begin{proof}
	...
	\begin{claim}
		For all \(i, k\in P^\prime \) such that \(\alpha _k \leq \alpha _i\), at time (right before) \(\alpha _i\), offer from \(k\) to \(j\) is at most \(\alpha _i - d(i, j) - 2d(k, j)\).
	\end{claim}
	\begin{explanation}
		We see that if \(\alpha _i = \alpha _k\), the offer is just \((\alpha _k - d(i, j))^+\). Otherwise, we have \(\alpha _k < \alpha _i\). If \(\alpha _i > d(k, j^\prime ) + d(k, j) + d(i, j)\), we immediately get a contradiction. Hence,
		\[
			\alpha _i \leq d(k, j^\prime )+ d(k, j) + d(i, j),
		\]
		i.e., the offer is \((d(k, j^\prime ) - d(k, j))^+ \geq \alpha _i - d(i, j) - 2d(k, j)\).
	\end{explanation}
	...
	\end{equation}
	since we can consider the total offer to \(j\) at time (right before) \(\alpha _i\). Now, we add \autoref{eq:lec6} for all \(i\in [n]\), we have
	\[
		n \sum_{i=1} ^n \alpha _i - \underbrace{\sum_{i=1} ^n (i-1)d(i, j) - 2 \sum_{i=1} ^n (n - i)d(i, j) - \sum_{i=1} ^n i\cdot d(i, j)}_{\geq \sum_{i=1} ^n (i\cdot d(i, j) + 2(n-i)d(i, j) + id(i, j)) = \sum_{i=1} ^n 2n\cdot d(i, j)} \leq n \hat{f} = n \hat{f} = 2n f.
	\]
