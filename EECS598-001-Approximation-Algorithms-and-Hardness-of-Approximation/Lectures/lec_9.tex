...
\begin{note}
	...
	\]
	Furthermore, in the case of \(\delta =2\), we see that we can set \(\rho \coloneqq 1 + \delta = 3\).
\end{note}
...
\begin{explanation}
	...
	\]
	\begin{remark}[Fact]
		One can actually show that \(i^\prime \) (i.e., the geometric mean) is the optimal solution for \hyperref[prb:Euclidean-k-median]{\(k\)-means}, and if we choose \(i\) rather than \(i^\prime \) to be the center, the deviation from \(\OPT\) is exactly \(\left\vert S \right\vert \left\lVert i^\prime - i\right\rVert ^{2} \).
	\end{remark}
	Also,
	\[
		\begin{split}
...
Now, given the fact that we already handle the case that \(\left\vert S \right\vert = 0\) and \( \left\vert S \right\vert = 1\), we now consider the cases that \(\left\vert S \right\vert = 2\) and \(\left\vert S \right\vert = 3\). If \(\left\vert S \right\vert = 2\), let \(S = \left\{ j_1, j_2 \right\}\), then \((\alpha _i - d(i, j_1)) + (\alpha _i - d(i, j_2)) \leq (2 - \delta )\alpha _i\). Since \(\mathop{\mathrm{conn}}(i) \leq \alpha _i\),
...
\begin{remark}[SOTA]
	...
	\[\begin{tikzcd}
			...
			{\text{\hyperref[algo:Euclidean-k-median-PD]{Euclidean Primal-Dual} \(2.63\)-\hyperref[def:LMP]{LMP}}} && {\text{\(2.63\)-approximation}} \\
