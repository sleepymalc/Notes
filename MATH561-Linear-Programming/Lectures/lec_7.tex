...
\begin{algorithm}[H]\label{algo:worry-free-simplex-algorithm}
	...
	\KwData{A \hyperref[def:standard-form]{standard form} \((\mathrm{P})\), \hyperref[def:basic-partition]{basic partition} \(\beta , \eta\) with \(x_{\beta}\geq 0\)}
	\KwResult{\hyperref[def:optimal-solution]{Optimal solutions} \(\overline{x}, \overline{y}\), or report \((\mathrm{P})\) is unbounded}
	\BlankLine

	\While{\(\True\)}{
	\(\overline{x}_{\beta} \gets A^{-1}_{\beta} b(\geq 0)\)\;
	\(\overline{c}_{\eta}^{\top} \gets c_{\eta}^{\top} - c_{\beta}^{\top} A^{-1}_{\beta} A_{\eta}\)\;
	\uIf(\Comment*[f]{\(\overline{x}\) is \hyperref[def:optimal-solution]{optimal} for \((\mathrm{P})\)}){\(\overline{c}_{\eta} \geq 0\)}{
	...
	\If(\Comment*[f]{\(\overline{A}_{\eta_{j}} \leq 0\implies (\mathrm{P}) \text{ is unbounded}\)}){\(\overline{A}_{\eta_{j}} \leq 0\)}{
	\Return{\((\mathrm{P})\) is unbounded}
	}
