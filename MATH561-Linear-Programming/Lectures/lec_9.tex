...
\begin{algorithm}[H]\label{algo:simplex-algorithm}
	...
	\KwData{\hyperref[def:standard-form]{standard form} LP \((\mathrm{P})\)}
	\KwResult{\hyperref[def:optimal-solution]{optimal solutions} \(\overline{x}, \overline{y}\), or report \((\mathrm{P})\) is unbounded/infeasible}
	...
	\uIf{\result is unbounded}{
		\Return{\((\mathrm{P})\) has no solution}
	}
	\Else{
		\(\beta\gets\result\)\Comment*[f]{Retrieve the feasible basis for \((\mathrm{P})\)}
	}
	\;
...
\begin{table}[H]
	...
	\begin{tabular}{c|c|c|c|c}
		\toprule
		\hyperref[algo:simplex-algorithm]{\emph{Simplex Algorithm}}                                                         & \((\mathrm{P}) \backslash (\mathrm{D})\)                                                                                 & \begin{tabular}{@{}c@{}}\hyperref[def:optimal-solution]{optimal}\\\hyperref[def:optimal-solution]{solution}\end{tabular} & infeasible  & unbounded   \\
		\midrule
		...
		\bottomrule
	\end{tabular}
	\caption{Comparison between \((\mathrm{P})\) and \((\mathrm{D})\)}
	\label{tab:label}
\end{table}