...
\begin{definition}[Cellular chain complex]\label{def:cellular-chain-complex}
	...
	\begin{definition}[Cellular chain group]\label{def:cellular-chain-group}
		...
		\[
			C_{n} (X) \coloneqq \mathbb{Z} \langle e^n_\alpha \mid e^n_\alpha \text{ an \hyperref[def:cell]{\(n\)-cells} of \(X\)}\rangle (\cong H_n(X^n, X^{n-1}))
		\]
...
\end{definition}
\begin{remark}
	We sometimes denote the \hyperref[def:cellular-homology-group]{cellular homology group} as \(H_n^{\CW }(X) \) if it causes confusion.
\end{remark}
...
\begin{eg}
	...
	\begin{enumerate}[(a)]
		\item \(S^n, n \geq 2\). Since if we have \(S^n\) with \(n \geq 2\), using the \hyperref[def:CW-Complex]{CW complex} structure of \(e^n\) attached to a single point \(x_0\). The \hyperref[def:cellular-chain-complex]{cellular chain complex} is given as
		      \[
			      \begin{tikzcd}
				      0 & 0 & {\langle e^n\rangle} & 0 & \dots & 0 & {\langle x_0\rangle} & 0
		      ...
		      \[
			      ...
			      \end{dcases}
		      \]
		\item \(\mathbb{C} P^n, \forall n\). In this case, we can let \(\mathbb{C} P^n\) equipped with a \hyperref[def:CW-Complex]{CW complex} structure with one \hyperref[def:cell]{cell} of each even dimension \(2k \leq 2n\), thus
		      \[
			      ...
			      \end{dcases}
		      \]
		\item \(S^n \times S^n, n > 1\). We let \(S^n \times S^n\) has the \hyperref[CW-complex-product]{product CW structure} consisting of a \hyperref[def:cell]{\(0\)-cell}, two \hyperref[def:cell]{\(n\)-cells}, and a \hyperref[def:cell]{\(2n\)-cell}. Thus, we have
		      \[
			      H_{k}(\mathbb{C} P^n) \cong \begin{dcases}
