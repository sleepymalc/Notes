...
\begin{proof}
	...
	\begin{definition}[Word]\label{def:word}
		Fix a set \(S\), and we define a \emph{word} as a finite sequence (possibly \(\varnothing \)) in the formal symbols
		\[
...
The \hyperref[def:fundamental-group]{fundamental group} is the first and the simplest \hyperref[def:homotopy]{homotopy} group, which records information about the basic shape, or holes, of the topological space. Not surprisingly, this is already interesting enough to study and can help us distinguish between different shapes just by finding the spaces' corresponding \hyperref[def:fundamental-group]{fundamental groups}.
...
\begin{eg}
	Fix \(x_1, x_0\in X\), then \underline{\(\exists\) \hyperref[def:homotopy]{homotopy} of \hyperref[def:path]{paths}} is an equivalence relation on \hyperref[def:path]{paths} from \(x_0\) to \(x_1\) (i.e., \(\gamma\) with \(\gamma(0)=x_0, \gamma(1)=x_1\)).
\end{eg}

\begin{definition}[Path composition]\label{def:path-composition}
	For \hyperref[def:path]{paths} \(\alpha , \beta \) in \(X\) with \(\alpha (1) = \beta (0)\), the \emph{composition}\footnote{Also named \emph{product}, \emph{concatenation}.} \(\alpha \cdot \beta \) is
	\[
...
\begin{exercise}
	A \hyperref[def:path]{path} \(\gamma\) is \hyperref[def:homotopy-relative]{homotopic \(\rel \{0, 1\}\)} to all of its \hyperref[def:reparameterization]{reparameterizations}.
\end{exercise}
\begin{answer}
	We show that \(\gamma\) and \(\gamma\circ \phi \) are \hyperref[def:homotopic]{homotopic} \(\rel \{0, 1\}\) by showing that there exists a continuous \(F_t\) such that
	\[
	...
	Now, we check that \(F_t\) really gives us a \hyperref[def:homotopy-relative]{homotopic \(\rel \{0, 1\}\)}. We have
	\[
		\begin{split}
...
\begin{exercise}
	Fix \(x_1, x_1\in X\). Then \hyperref[def:homotopy-path]{\underline{homotopy of paths}} (\hyperref[def:homotopy-relative]{relative \(\{0, 1\}\)}) is an equivalence relation on \hyperref[def:path]{paths} from \(x_0\) to \(x_1\).
\end{exercise}

\section{Fundamental Group and Groupoid}
In class, we only discuss \hyperref[def:fundamental-group]{fundamental groups}. But there is an also important concept called \hyperref[def:fundamental-groupoid]{fundamental groupoid}, which is kind of similar but more general and power to use. I include this just for completeness, feel free to skip the additional content. Now, let's start with \hyperref[def:fundamental-group]{fundamental group}.

\subsection{Fundamental Group}
\begin{definition}[Fundamental group]\label{def:fundamental-group}
	Let \(X\) denotes the space and let \(x_0\in X\) be the base point. The \emph{fundamental group of \(X\) based at \(x_0\)}, denoted by \(\pi_1(X, x_0)\), is a group such that
	\begin{itemize}
		\item Elements: \hyperref[def:homotopy]{Homotopy} classes \(\rel \{0, 1\}\) of \hyperref[def:path]{paths} \([\gamma]\) where \(\gamma\) is a \textbf{loop} with \(\gamma(0) = \gamma(1) = x_0\)\footnote{We say \(\gamma\) is \textbf{based} at \(x_0\).}
		      \begin{center}
...
\begin{proof}
	...
	\[
		...
		\end{dcases}
	\]
	Then, we define \(\phi \colon I\to I\) such that
	\[
		\phi (t) = \begin{dcases}
	...
	\paragraph{Inverses.} We want to show that \(\gamma\cdot \overline{\gamma} \simeq c\), where \(\overline{\gamma} (t) = \gamma(1-t)\). Firstly, we have
	\[
...
\end{theorem}
\begin{proof}
	To show that the \emph{change-of-basepoint map} is isomorphism, we show that it's one-to-one and onto.
	\begin{itemize}
		\item One-to-one. Consider that if \([h\cdot \gamma\cdot \overline{h} ] = [h\cdot \gamma ^\prime \cdot \overline{h} ]\), then since we know that \(h^{-1}  = \overline{h} \), hence in the \hyperref[def:fundamental-group]{fundamental group} \(\pi _1(X, x_0)\), we see that
		      \[
	...
	We then see that the \hyperref[def:fundamental-group]{fundamental group} of \(X\) does not depend on the choice of basepoint, only on the choice of the \hyperref[def:path]{path} component of the basepoint. If \(X\) is \hyperref[def:path]{path}-connected, it now makes sense to refer to \emph{the} \hyperref[def:fundamental-group]{fundamental group} of \(X\) and write \(\pi _1(X)\) for the abstract group (up to isomorphism).
\end{proof}
...
\begin{lemma}\label{lma:lec8-1}
	Given \(x_0, x_1, x_2\in X\), \(\alpha , \alpha ^\prime\) are two \hyperref[def:path]{paths} from \(x_0\) to \(x_1\), and \(\beta , \beta ^\prime \) are two \hyperref[def:path]{paths} from \(x_1\) to \(x_2\). If \(\langle \alpha  \rangle = \langle \alpha ^\prime  \rangle \), \(\langle \beta  \rangle = \langle \beta ^\prime  \rangle \), then \(\langle \alpha \cdot \beta  \rangle = \langle \alpha ^\prime \cdot \beta ^\prime  \rangle\).
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lma:lec8-2}
	...
	\[
		\langle (\alpha \cdot \beta ) \cdot \gamma \rangle  = \langle \alpha \cdot (\beta \cdot \gamma ) \rangle.
	\]
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lma:lec8-3}
	Let \(X\) be a topological space, and \(x_0\in X\). Then for every \hyperref[def:homotopy-path]{path homotopy} \(\langle \alpha  \rangle \) from \(x_1\) to \(x_2\), we have
	\[
		\langle c_{x_1}\cdot \alpha  \rangle = \langle \alpha  \rangle = \langle \alpha \cdot c_{x_2} \rangle.
	\]
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lma:lec8-4}
	For every \hyperref[def:homotopy-path]{path homotopy} \(\langle \alpha  \rangle \) from \(x_1\) to \(x_2\), then
	\[
		\langle \alpha \cdot \alpha ^{-1}  \rangle = \langle c_{x_1} \rangle, \qquad \langle \alpha ^{-1} \cdot \alpha  \rangle = \langle c_{x_2} \rangle.
	\]
\end{lemma}
\begin{proof}
...
This section is not covered in class, but it's a useful concept. The idea is that after giving \autoref{def:fundamental-group}, we see that we actually create a \hyperref[def:fundamental-group]{fundamental group} at \textbf{every} point in \(X\), furthermore, when we use \autoref{thm:lec8} if \(X\) is \hyperref[def:path]{path}-connected, we actually \textbf{lose} some information about this space. Here is how we can store all the information.
...
\begin{definition}[Groupoid]\label{def:groupoid}
	A \hyperref[def:category]{category} \(\mathscr{C}\) is a \emph{groupoid} if any \hyperref[def:morphism]{morphisms} in \(\mathscr{C}\) is and isomorphism.
\end{definition}
		...
		\item Composition: For every \(p, q, r\in \Ob (\Pi (X)) = X\),
		      \[
			      \begin{split}
				      ...
				      (\langle \alpha  \rangle , \langle \beta  \rangle )            & \mapsto \langle \beta  \rangle \circ \langle \alpha  \rangle \coloneqq \langle \alpha \cdot \beta  \rangle.
			      \end{split}
		      \]
		\item Identity: For every \(p\in \Ob (\Pi (X)) = X\), we define \(1_p\coloneqq \langle c_p \rangle \in \Hom _{\Pi (X)}(p, p)\) be the constant loop based at \(p\) such that for every \(\langle \alpha \rangle \in \Hom _{\Pi (X)}(p, q)\),
		      \[
			      \langle \alpha  \rangle \circ \id_{p} = \id_{q}\circ \langle \alpha  \rangle = \langle \alpha  \rangle.
		      \]
		\item Associativity: Given \(p, q, r, s\in \Ob (\Pi (X)) = X\), with the \hyperref[def:path]{paths}
		      \[
			      \begin{tikzcd}
				      p & q & r & s
				      \arrow["{\langle\alpha\rangle}", curve={height=-6pt}, from=1-1, to=1-2]
				      \arrow["{\langle\beta\rangle}", curve={height=6pt}, from=1-2, to=1-3]
				      \arrow["{\langle\gamma\rangle}", curve={height=-6pt}, from=1-3, to=1-4]
			      \end{tikzcd}
		      \]
		      Then
		      \[
			      \langle \gamma  \rangle \circ \left(\langle \beta  \rangle \circ \langle \alpha  \rangle \right) = \left(\langle \gamma  \rangle \circ \langle \beta  \rangle \right)\circ \langle \alpha  \rangle.
		      \]
	\end{itemize}
\end{definition}
\begin{proof}
	...
	\begin{itemize}
		...
		\item Identity: It follows that
		      \[
			      \langle \alpha  \rangle \circ \id_{p} = \langle c_p\cdot \alpha  \rangle = \langle \alpha  \rangle
		      \]
	...
	\[
		\begin{dcases}
			\langle \alpha ^{-1} \cdot \alpha  \rangle & = \langle c_q \rangle \eqqcolon \id_{q}  \\
			\langle \alpha \cdot \alpha^{-1}  \rangle  & = \langle c_p \rangle \eqqcolon \id_{p}.
		\end{dcases}
	\]
	Furthermore, since \(\langle \alpha ^{-1} \cdot \alpha  \rangle = \langle \alpha  \rangle \circ \langle \alpha ^{-1}  \rangle \) and \(\langle \alpha \cdot \alpha^{-1} \rangle = \langle \alpha ^{-1}\rangle \circ \langle \alpha\rangle \), hence this means \(\Pi (X)\) is indeed a \hyperref[def:groupoid]{groupoid}.
\end{proof}
...
\begin{remark}
	...
	\]
	Hence, we only need to verify their group composition agrees. But this is trivial, since for every two \(\langle \alpha  \rangle , \langle \beta  \rangle \in \Aut_{\Pi (X)}(p) \),
	\[
		\underbrace{\langle \alpha  \rangle \cdot \langle \beta  \rangle }_{\text{Composition from \(\Aut_{\Pi(X)}\)}} = \langle \beta  \rangle \circ \langle \alpha  \rangle =\underbrace{ \langle \alpha \cdot \beta\rangle}_{\text{Composition from \(\pi_1\) } }.
	\]
