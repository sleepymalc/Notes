...
\begin{corollary}[The fundamental group of \(S^k\)]
	The torus \(T\cong S^{1}\times S^1\) has \hyperref[def:fundamental-group]{fundamental group} \(\pi _1(T)\cong \mathbb{Z} ^2\). Additionally,
	for a \(k\)-torus
	\[
		\underbrace{S^{1}\times S^1 \times \dots \times S^1}_{k\text{ times}} = (S^1)^k,
	\]
	the \hyperref[def:fundamental-group]{fundamental group}
	is then \(\mathbb{Z} ^k\), i.e.
	\[
		\pi _1\left((S^1)^k\right) \cong \mathbb{Z} ^k.
	\]
\end{corollary}
\begin{proof}[Proof idea]
	Since
	\[
		\pi _1 \cong \mathbb{Z}^2 \cong \mathbb{Z} _a \oplus \mathbb{Z} _b.
	\]
	\begin{figure}[H]
		\centering
		...
		\item on \hyperref[def:morphism]{morphisms}: for every \(X, Y\in \Ob (\underline{\Top})\), \(f\colon X\to Y\), define a \hyperref[def:functor]{functor}
		      ...
		      \begin{itemize}
			      \item on \hyperref[def:object]{objects}: For every \(p\in \Ob (\Pi (X)) = X\), \(\Pi (f)(p) = f(p)\). i.e.,
			            ...
			            \]
			      \item on \hyperref[def:morphism]{morphisms}: For every \(\left< \alpha  \right> \in \Hom _{\Pi (X)}(p, q)\), define
			            \[
				            \Pi (f)(\left< \alpha  \right> )\coloneqq \left< f\circ \alpha  \right> \in \Hom _{\Pi (Y)}(f(p), f(q)).
			            \]
		      \end{itemize}
	\end{itemize}
\end{theorem}
\begin{proof}
	...
	\begin{itemize}
		\item \(\Pi (f)(\id_{p} ) = \id_{f(p)} \). Indeed, since
		      \[
			      \Pi (f)(\id_{p} ) = \Pi (f)(\left< c_p \right> ) = \left< f\circ d_{p}  \right> = \left< c_{f(p)} \right> = \id_{f(p)}.
		      \]
	...
	\begin{itemize}
		\item \(\Pi (\id_{X} ) = \id_{\Pi (X)} \) for all \(X\in \Ob (\underline{\Top})\). This is trivial since
		      ...
		      \begin{itemize}
			      ...
			      \item on \hyperref[def:morphism]{morphisms}: \(\begin{tikzcd}
				            {p} & {q}
				            \arrow["\left< \alpha  \right> ", from=1-1, to=1-2]
			            \end{tikzcd}\mapsto \left< \id_{X} \circ \alpha  \right> = \left< \alpha  \right>  \).
		      \end{itemize}
		\item For all \(X, Y, Z\in \Ob (\underline{\Top })\),
		      ...
		      \begin{itemize}
			      ...
			      \item on \hyperref[def:morphism]{morphisms}: For all \(\left< \alpha  \right> \in \Hom _{\Pi (X)}(p, q)\),
			            \begin{itemize}
