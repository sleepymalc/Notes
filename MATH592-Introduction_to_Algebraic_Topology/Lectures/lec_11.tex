...
\begin{eg}
	...
	\begin{enumerate}
		\item We can use \hyperref[thm:Seifert-Van-Kampen-Theorem]{Seifert Van Kampen Theorem} to compute the fundamental group of \(S^{2}\). We see that
		      \begin{figure}[H]
			      \centering
		      ...
		      \begin{exercise}
			      Suppose we have path connected spaces \((X_\alpha , x_\alpha )\), and we take their wedge sum \(\bigvee_\alpha X_\alpha \) by
		      ...
		      \begin{corollary}
			      The wedge sum of circles \(\pi _1(\bigvee_{\alpha \in A}S^1) = \ast_\alpha \mathbb{\MakeUppercase{z}} \) is a \hyperref[def:free-group]{free group} on \(A\).
		...
		\item Let's see the last example which illustrate the power of \hyperref[thm:Seifert-Van-Kampen-Theorem]{Seifert Van Kampen Theorem}. Start with a torus, and
		      ...
		      space so that each disk is a \hyperref[def:CW-subcomplex]{subcomplex}. Then, we take quotient of each disk to a point without changing the homotopy type,
		      hence \(X\) is homotopy to
		      \begin{figure}[H]
			      \centering
			      \incfig{lec11:eg-3:2}
			      \label{fig:lec11:eg-3:2}
		      \end{figure}
		      By the same property, we can expand one of those points into an interval, and then contract the red path as follows.
		      \begin{figure}[H]
			      \centering
			      \incfig{lec11:eg-3:3}
...
\begin{exercise}
	...
	\[
		\pi _1(X)\simeq F_n.
	\]
	One way to do this is to convince yourself that you can do a \hyperref[def:deformation-retraction]{deformation retract} the plane onto the following wedge.
	\begin{figure}[H]
		\centering
		\incfig{lec11:ex}
