...
\begin{definition}[Regular (normal) cover]\label{def:regular}\label{def:normal}
...
\begin{eg}[Regular and non-regular cover of \(S^1\vee S^1\)]
	Given the following \hyperref[def:covering-map]{covers} of \(S^1\vee S^1\), determine which cover is \hyperref[def:regular]{regular}.
	\begin{figure}[H]
		\centering
...
\end{eg}
\begin{explanation}
	The left one is \hyperref[def:regular]{regular}, while the right one is not since there is no automorphism from \(\widetilde{x} _0\) to
