...
\autoref{def:chain-complex} in \autoref{lma:lec21}. But we note that the definition of \(C_n\) and \(d_n\) can be further abstract, hence
we give the following definitions.

\begin{definition}[Chain group]\label{def:chain-group}
	We call \(C_{n} \) the \emph{\(n\)-th chain group} in \autoref{def:chain-complex}.
\end{definition}
\begin{definition}[Differential]\label{def:differential}
	We call \(d_{n} \) the \emph{\(n\)-th differential} in \autoref{def:chain-complex}.
\end{definition}
\begin{note}
	Note that \autoref{def:chain-complex} is purely \emph{abstract}, namely we can put different \hyperref[def:chain-group]{chain group}
