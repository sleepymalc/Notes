\lecture{21}{23 Feb. 10:00}{Homology}
\subsection{Homology}
...
structure. Then, we define \(C_{n} (X)\) to be the free Abelian group on the \hyperref[def:standard-simplex]{\(n\)-simplices} of \(X\). That is,
\[
...
\begin{definition}[Chain group]\label{def:chain-group}
	We define the \emph{chain group} \(C_{n} (X)\) of order \(n\) to be the free Abelian group on the \hyperref[def:standard-simplex]{\(n\)-simplices} of \(X\) such
	that
	\[
...
\end{lemma}
\begin{proof}
	Since all \(C_{i} \) are free Abelian group, hence we only need to consider \(\partial _{n-1}\circ \partial _{n} (\sigma ) = 0\) for a generator \(\sigma \). Given a generator \(\sigma \)
\end{proof}
...
\begin{definition}[Chain complex]\label{def:chain-complex}
	...
	\[
		...
		\end{tikzcd}
	\]
	of Abelian groups and group homomorphism such that
	\[
		d_{n-1}\circ d_n = 0.
	\]
...
\begin{remark}
	...
	\begin{itemize}
		...
		\item \autoref{def:chain-complex} means that \(\mathrm{ker}(d_{n} ) \) contains \(\mathrm{Im} (d_{n+1})\), since \(d_{n} \circ d_{n+1} = 0\).
	\end{itemize}
\end{remark}

\begin{definition}[Exact]\label{def:exact}
	We say that the sequence is \emph{exact at \(C_{n} \)} provided that \(\mathrm{ker}(d_{n}) = \mathrm{Im}(d_{n+1})\). A \hyperref[def:chain-complex]{chain complex} is \emph{exact}
...
\begin{definition}[Homology group]\label{def:homology-group}
	...
	\[
		H_{n} \coloneqq \quotient{\mathrm{ker} (d_{n} )}{\mathrm{Im} (d_{n+1})}.
	\]
\end{definition}
\begin{remark}
...
With what we have just defined, it's natural to define \hyperref[def:homology-group]{homology groups} of spaces \(X\) with a \hyperref[def:delta-complex]{\(\Delta \)-complex}
structure.
\begin{definition}[Homology class]\label{def:homology-class}
	We say \(\mathrm{ker} (\partial _{n} )\) is the subgroup of \textbf{cycles} is \(C_{n} (X)\) , and \(\mathrm{Im}(\partial_{n+1}) \) is the subgroup of \textbf{boundaries}
	in \(C_{n} (X)\). We then set
	\[
		H_{n} (X)\coloneqq \quotient{\mathrm{ker} (\partial _{n} )}{\mathrm{Im} (\partial _{n+1})} = \quotient{\text{cycles} }{\text{boundaries} }.
	\]
