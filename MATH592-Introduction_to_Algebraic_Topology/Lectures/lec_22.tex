...
\begin{enumerate}
	\item Choose \hyperref[def:delta-complex]{\(\Delta\)-complex} structure on \(X\). (Later: We will prove \(H_\ast(X)\) is independent of the choice of \hyperref[def:delta-complex]{\(\Delta\)-complex} structure)
	...
	\item For each \(n\) compute \(H_n(X)= \quotient{\ker \partial_n}{\im \partial_{n + 1}}\). Be careful that any change-of-variables map you apply is invertible over $\mathbb{\MakeUppercase{z}} $.
\end{enumerate}