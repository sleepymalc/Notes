...
\begin{proof}
	For 1., we see that
	\[
		\quotient{X^n}{X^{n-1}} \cong \text{\hyperref[sssec:Wedge-sum]{wedge} of one \(n\)-sphere for each \(n\)-cell}.
	\]

	Now we prove 2. and 3., We consider the long \hyperref[def:exact-sequence]{exact sequence} of a \hyperref[def:good-pair]{pair} for fixed \(n\),
	\[
		\begin{tikzcd}
	...
	\[
		\begin{tikzcd}
			...
			\arrow["\cong", color={rgb,255:red,214;green,92;blue,92}, from=1-4, to=2-2]
			\arrow["{\text{inj.}}", from=2-2, to=2-3]
			\arrow["{\text{surj.}}", from=2-3, to=2-4]
			\arrow["\cong"{description}, color={rgb,255:red,214;green,92;blue,92}, from=2-4, to=3-2]
			\arrow["\cong", color={rgb,255:red,214;green,92;blue,92}, from=3-2, to=3-3]
			\arrow["\cong", color={rgb,255:red,214;green,92;blue,92}, from=3-3, to=3-4]
			\arrow["\cong", color={rgb,255:red,214;green,92;blue,92}, from=1-1, to=1-2]
	...
	\begin{note}
		...
		from our analysis of the long \hyperref[def:exact-sequence]{exact sequence} of a \hyperref[def:good-pair]{pair} above.
	\end{note}
...
\begin{proof}[Proof of \autoref{thm:cellular-homology-agrees-with-singular-homology}]
	...
	\[
		...
		\end{tikzcd}
	\]
	These come from the long exact sequences of a pair combined with the things we've deduced in the \hyperref[lma:lec-34]{preliminaries}.
	We can paste these together into a diagram, we have
	...
	Then this has to at least be a \hyperref[def:chain-complex]{chain complex} since the diagram commutes because of Hatcher\cite{hatcher2002algebraic}. We then know that
	\[
	...
	\end{align*}
	\(j_{n - 1}\) must also inject by \hyperref[def:exact]{exactness}, and so applying \hyperref[def:exact]{exactness}:
	\[
