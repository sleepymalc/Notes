...
\begin{definition}[Category, object, morphism]\label{def:category}\label{def:object}\label{def:morphism}
...
\begin{definition}[monic, epic]\label{def:monic}\label{def:epic}
...
\begin{definition}[Functor]\label{def:functor}
	Given \(\mathscr{C} , \mathscr{D} \) be two \hyperref[def:category]{categories}. A (\underline{covariant}) \emph{functor}
	\[
