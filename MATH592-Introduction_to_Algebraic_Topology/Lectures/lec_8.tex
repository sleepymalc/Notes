...
at \textbf{every} point at \(X\), furthermore, when we use \autoref{thm:lec8} if \(x\) is \hyperref[def:path]{path}-connected, we actually \textbf{lose} some
...
\begin{definition}[Groupoid]\label{def:groupoid}
	A \hyperref[def:category]{category} \(\mathcal{\MakeUppercase{C}}\) is a \emph{groupoid} if any \hyperref[def:morphism]{morphisms} in \(\mathcal{\MakeUppercase{C}} \)
	is and isomorphism.
\end{definition}
\begin{remark}
...
\begin{definition}[Fundamental groupoid]\label{def:fundamental-groupoid}
	Let \(X\) denotes the space, then the category \(\Pi (X)\) is a \emph{fundamental groupoid of \(X\)} such that
	\begin{itemize}
		\item \(\Object (\Pi (X)) \coloneqq X \)
		\item \(\Morphism (\Pi (X))\colon \forall p, q\in \Object (\Pi (X)) = X\),
		      \[
		...
		\item Identity: For every \(p\in \Object (\Pi (X)) = X\), we define \(1_p\coloneqq \left< c_p \right> \in \Homomorphism _{\Pi (X)}(p, p)\) be the constant loop
		      ...
		      \[
			      \left< \alpha  \right> \circ 1_p = 1_q\circ \left< \alpha  \right> = \left< \alpha  \right>.
		      \]
		\item Associativity: Given \(p, q, r, s\in \Object (\Pi (X)) = X\), with the paths
		      \[\begin{tikzcd}
				      p & q & r & s
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Identity: It follows that
		      \[
			      \left< \alpha  \right> \circ 1_p = \left< c_p\cdot \alpha  \right> = \left< \alpha  \right>
		      \]
	...
	Additionally, from \autoref{lma:lec8-4}, we see that
	\[
		\begin{dcases}
			\left< \alpha ^{-1} \cdot \alpha  \right> & = \left< c_q \right> \eqqcolon 1_q  \\
			\left< \alpha \cdot \alpha^{-1}  \right>  & = \left< c_p \right> \eqqcolon 1_p.
		\end{dcases}
	\]
...
\begin{remark}
	Assume \(\mathcal{\MakeUppercase{C}} \) is a \hyperref[def:groupoid]{groupoid}, then for every \(x\in \Object (\mathcal{\MakeUppercase{C}} )\), we can define
	\[
		\cdot \colon \Homomorphism _{\mathcal{\MakeUppercase{C}} }(X, X) \times \Homomorphism _{\mathcal{\MakeUppercase{C}} }(X, X)\to \Homomorphism _{\mathcal{\MakeUppercase{C}} }(X, X)
	\]
	such that
	\[
	...
	We can prove that
	\[
		\left(\Homomorphism _{\mathcal{\MakeUppercase{C}} }(X, X), \cdot \right)
	\]
	defines a group \(\mathrm{Aut}_{\mathcal{\MakeUppercase{C}} }(x) \) called the \emph{isotropy group} of \(\mathcal{\MakeUppercase{C}} \) at \(x\in X\).
\end{remark}
\begin{exercise}
	For every \(x, y\in \Object (\mathcal{\MakeUppercase{C}} )\), if there exists \(f\in \Homomorphism _{\mathcal{\MakeUppercase{C}} }(x, y)\), then \(f\) induces
	\[
		f_\ast \colon \mathrm{Aut}_{\mathcal{\MakeUppercase{C}} }(x) \overset{\simeq}{\to } \mathrm{Aut}_{\mathcal{\MakeUppercase{C}} }(y),
	\]
