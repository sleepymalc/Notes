...
\begin{corollary}
	If \(f\colon X\to \mathbb{C} \) is a \hyperref[def:measurable-function]{measurable function} if and only if there exists \hyperref[def:simple-function]{simple functions} \(\phi _{n}\colon X\to \mathbb{C}\) such that
	\[
...
\begin{definition}[Integration of nonnegative simple funciton]\label{def:integration-of-nonnegative-simple-function}
	Let \((X, \mathcal{A}, \mu  )\) be a \hyperref[def:measure-space]{measure space}, and \(\phi\colon X\to [0, \infty ]\) such that \(\phi  = \sum_{i=1}^{N} c_{i}\mathbbm{1}_{E_{i}}\) is a \hyperref[def:simple-function]{simple function}. We define its \emph{integration} to be
	\[
...
\begin{proposition}
	...
	\begin{enumerate}[(a)]
		\item \autoref{def:integration-of-nonnegative-simple-function} is well-defined.
...
\begin{definition}[Integration of nonnegative function]\label{def:integration-of-nonnegative-function}
	Given a \hyperref[def:measure-space]{measure space} \((X, \mathcal{A} , \mu )\) with \(f\colon X\to [0, \infty ]\) be \hyperref[def:A-measurable-function]{\(\mathcal{A}\)-measurable}, we define its \emph{integration} to be
	\[
...
\begin{note}
	Note that
	\begin{itemize}
		\item If \(f\) is a \hyperref[def:simple-function]{simple function}, the \autoref{def:integration-of-nonnegative-simple-function} and \autoref{def:integration-of-nonnegative-function} of \(\int_{}^{} f \) are the same.
...
\begin{proof}
	...
	\[
		\int (f+g)
		\overset{\hyperref[thm:MCT]{!}}{=}\lim_{n \to \infty}\int (\phi _{n} + \psi _{n})
		= \lim_{n \to \infty} \left( \int \phi _{n} + \int \psi _{n} \right)
		\overset{\hyperref[thm:MCT]{!}}{=} \int f + \int g.
	\]
\end{proof}
