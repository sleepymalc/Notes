...
\begin{definition}[Riemann integrable]\label{def:Riemann-integrable}
	A \underline{bounded} function \(f\colon [a, b]\to \mathbb{R} \) is called \emph{Riemann (Darboux) integrable} if \(\underline{I} = \overline{I}\). If so, then we write
	\[
...
\begin{theorem}
	...
	\begin{enumerate}[(a)]
		\item If \(f\) is \hyperref[def:Riemann-integrable]{Reimann integrable}, then \(f\) is \hyperref[def:measurable-function]{Lebesgue measurable}, thus \hyperref[def:integrable]{Lebesgue integrable}. Further,
		      \[
			      \int_{a}^{b} f(x) \,\mathrm{d}x = \int_{[a, b]}f \,\mathrm{d}m.
		      \]
		\item If \(f\) is \hyperref[def:Riemann-integrable]{Reimann integrable} \(\iff \) \(f\) is continuous \underline{\hyperref[def:Lebesgue-measure]{Lebesgue} \hyperref[def:mu-almost-everywhere]{a.e.}}\footnote{Here, we mean that the set where \(f\) is discontinuous is a \hyperref[def:null-set]{null set} under \hyperref[def:Lebesgue-measure]{Lebesgue measure}.}
	\end{enumerate}
\end{theorem}
\begin{proof}
	...
	\end{itemize}
	if \(P_{n} = \{a = t_0 < t_1 < \dots < t_{K} \}\). Let \(\phi \coloneqq \sup_n \phi _{n}\) and \(\psi \coloneqq \inf _n \psi _{n}\). We then see that \(\phi , \psi \) are \hyperref[def:measurable-function]{Lebesgue (Borel) measurable function}.
	\begin{note}
		Note that
		\begin{itemize}
		...
		\end{itemize}
	\end{note}
	By \hyperref[thm:DCT]{dominated convergence theorem} and the fact that \(M\mathbbm{1}_{[a, b]} \in L^1(\mathbb{R} , \mathcal{L} , m) \), we have
	\[
...
\begin{lemma}
	Let \(B_{n, k} \coloneqq \left\{x\in X \mid \left\vert f_{n}(x) - f(x) \right\vert < 1 / k \right\}\). Then
	\begin{enumerate}[(a)]
...
\begin{definition*}
	Let \((X, \mathcal{A} , \mu )\) be a \hyperref[def:measure-space]{measure space} and assume that \(f_{n}, f\) are \hyperref[def:measurable-function]{measurable functions}.

	\begin{definition}[Converge almost everywhere]\label{def:converge-almost-everywhere}
		\emph{\(f_{n}\to f\) almost everywhere} means
		\[
			\exists \text{ \hyperref[def:null-set]{null set} } E\text{ such that } f_{n}\to f \text{ \hyperref[def:pointwise-convergence]{pointwise} on } E^{c}.
		\]
	\end{definition}
...
\begin{eg}\label{eg:lec15}
	Given \((\mathbb{R} , \mathcal{L} , m)\) and let \(f = 0\). Consider the following functions.
	\begin{enumerate}[(a)]
		\item\label{eg:lec15-a} \(f_{n} = \mathbbm{1}_{(n, n+1)} \).
		\item\label{eg:lec15-b} \(f_{n} = \frac{1}{n}\mathbbm{1}_{(0, n)} \).
		\item\label{eg:lec15-c} \(f_{n} = n \mathbbm{1}_{(0, \frac{1}{n})} \).
		\item\label{eg:lec15-d} The \emph{typewriter function}.
...
\begin{explanation}
	...
	\begin{exercise}
		Classify in what senses do \autoref{eg:lec15-a}, \autoref{eg:lec15-b}, \autoref{eg:lec15-c} and the \hyperref[eg:lec15-d]{typewriter function} converge.
	\end{exercise}
\end{explanation}