...
\begin{definition*}
	Given a vector space \(V\) with either a \hyperref[def:norm]{norm} or an \hyperref[def:inner-product]{inner product}, we have the following.
...
\begin{proof}
	...
	\]
	Since \(\left\lVert x_n - x\right\rVert\to 0\) and \(\left\lVert y\right\rVert\) is constant in \(n\), from the queeze theorem, we have
	\[
...
\begin{lemma}[Pythagorean theorem]\label{thm:Pythagorean}
