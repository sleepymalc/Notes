...
\begin{proof}[Proof of \autoref{prop:outer-measure}]
	...
	\end{claim}
	\begin{explanation}
		Let \(A_1, A_2, \dots \in X \). If one of \(\mu^{\ast} (A_{n}) = \infty \), then result holds. So we may assume \(\mu^{\ast} (A_n)<\infty \) for all \(n\in \mathbb{N} \). Now, fix any \(\epsilon >0\), we will show that
		\[
			\mu^{\ast} \left(\bigcup_{n=1}^{\infty} A_n\right)\leq \sum_{n=1}^{\infty} \mu^{\ast} (A_n)+\epsilon.
		\]
		For each \(n\in\mathbb{N} \), \(\exists E_{n, 1}, E_{n, 2}, \dots \in \mathcal{E}\) such that \(\bigcup_{k=1}^{\infty} E_{n, k}\supset A_n\) and \(\mu^{\ast} (A_n) + \epsilon /2^n \geq \sum_{k=1}^{\infty} \rho(E_{n, k})\).
		\begin{remark}
			This is an important trick! We often set the error term as \(\epsilon / 2^n\) instead of \(\epsilon \) as in above to accommodate the summation over a countable set.
		\end{remark}
		...
		\[
			...
			\overset{\hyperref[thm:Tonelli-for-series]{\text{!}}}{=} \sum_{n=1}^{\infty} \sum_{k=1}^{\infty} \rho(E_{k, n})
...
\begin{theorem}[Carathéodory extension theorem]\label{thm:Caratheodory-extension}
		      ...
		      \begin{itemize}
			      ...
			      \item We now show \(\mathcal{A} \) is closed under countable unions.
			            ...
			            \begin{explanation}
				            ...
				            \begin{figure}[H]
					            \centering
					            \incfig{thm:Caratheodory-extension-1a}
