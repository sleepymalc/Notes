\lecture{16}{11 Feb. 11:00}{Product Measure}
...
\begin{proposition}[Fast \(L^1\) convergence leads to a.e. convergence]\label{prop:lec16:1}
	...
	functions on \(X\). Then
	\[
		\sum\limits_{n=1}^{\infty} \left\lVert f_{n} - f\right\rVert_1 < \infty \implies f_{n}\to f\ \hyperref[def:mu-almost-everywhere]{a.e.}
	\]
\end{proposition}
\begin{proof}
	Let
...
\begin{corollary}
	Given \(f_{n}\to f\) in \(L^1\), there exists a subsequence \(f_{n_{j}}\to f\) \hyperref[def:mu-almost-everywhere]{a.e.}
\end{corollary}
\begin{proof}
	Since
	\[
...
\begin{eg}
	...
	\]
	\(f_{n}\to 0\) \hyperref[def:converge-in-measure]{in measure}. (Recall that \(f_{n}\nrightarrow 0\) in \(L^1\))
\end{eg}

\begin{remark}
	We see that
	\[\begin{tikzcd}
			& {f_n\to f \text{ in }L^1} & {f_n\to f \text{ \hyperref[def:converge-in-measure]{in measure}}} \\
			{f_n\to f \text{ \hyperref[prop:lec16:1]{fast in} }L^1} \\
			& {f_n\to f\ \hyperref[def:mu-almost-everywhere]{a.e.}} & {\exists f_{n_j}\to f\ \hyperref[def:mu-almost-everywhere]{a.e.}}
			\arrow[Rightarrow, "/"{marking}, shift left=1, from=1-2, to=3-2]
			\arrow[Rightarrow, "/"{marking}, shift left=1, from=3-2, to=1-3]
			\arrow[Rightarrow, "{\text{Check}}", shift left=1, from=1-2, to=1-3]
			\arrow[Rightarrow, "/"{marking}, shift left=1, from=1-3, to=1-2]
			\arrow[Rightarrow, "/"{marking}, shift left=1, from=1-3, to=3-2]
			\arrow[Rightarrow, "/"{marking}, shift left=1, from=3-2, to=1-2]
...
\begin{definition}[Uniformly a.e., almost uniformly]\label{def:uniformly-almost-everywhere}\label{def:almost-uniformly}
...
Now we can see the formal definition.
...
\begin{definition}[Product \(\sigma\)-algebra]\label{def:product-sigma-algebra}
	...
	Then a \emph{product \hyperref[def:sigma-algebra]{\(\sigma\)-algebra}} on \(X = \prod\limits_{\alpha \in I} X_\alpha \) is
	\[
...
\begin{lemma}
	...
	\[
		\bigotimes_{i = 1}^{\infty}\mathcal{\MakeUppercase{a}} _{i} = \left< \left\{\prod\limits_{i = 1}^{\infty} E_{i} \mid E_{i}\in \mathcal{\MakeUppercase{a}} _{i}\right\} \right>.
	\]
\end{lemma}
\begin{proof}
	\todo{DIY}
