...
\begin{definition}
	...
	\[
		(\mathbb{R} ^d, \mathcal{B} (\mathbb{R} ^d), m\times \ldots \times m ),
	\]
	which is \underline{same} as the \emph{completion} of
	\[
		(\mathbb{R} ^d, \mathcal{L} \otimes \ldots \otimes \mathcal{L}, m\times \ldots \times m).
	\]
\end{definition}
...
\begin{remark}
	We see that
	\[
		\mathcal{L} ^d \supsetneq \mathcal{L} \otimes \ldots \otimes \mathcal{L}
...
\begin{lemma}[Vitali-type covering lemma]\label{lma:Vitali-type-covering-lemma}
	Let \(B_1, \ldots , B_k \) be a finite collection of open balls in \(\mathbb{R} ^d\). Then there
