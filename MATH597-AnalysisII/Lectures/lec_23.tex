...
\begin{remark}
	...
	\[
		\ell ^p\coloneqq \ell ^p(\mathbb{N} ) = \left\{a = (a_1, a_2, \ldots  )\mid \left\lVert a\right\rVert _p = \left(\sum\limits_{i=1}^{\infty} \left\vert a_{i} \right\vert^p \right)^{1/p}< \infty \right\}.
	\]
\end{remark}
...
\end{proof}
\begin{eg}
	For \(p = q = 2\), \(X = \{1, \ldots , d \}\) with \(\mu \) being the \hyperref[eg:counting-measure]{counting measure}, then for any \(x, y\in \mathbb{R} ^d\), we have
	\[
