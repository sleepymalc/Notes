...
\begin{proof}
	Suppose \(T_n\) is a \hyperref[def:Cauchy-sequence]{Cauchy sequence} in \(L(V,W)\). Fix \(v \in V\), then let \(w_n = T_n v \in W\). Also,
	\[
		\left\lVert w_n - w_m\right\rVert = \left\lVert T_n v - T_m v\right\rVert = \left\lVert (T_n - T_m)v\right\rVert \leq \left\lVert T_n - T_m\right\rVert \left\lVert v\right\rVert.
	\]
...
\subsection{Dual of \(L^p\) Spaces}
\begin{eg}
	Let \(w \in \mathbb{\MakeUppercase{r}} ^d\). Then we can consider
	\[
		\max\{v \cdot w \mid \left\lVert v\right\rVert_2 = 1\} = \left\lVert w\right\rVert_2.
	\]
	If \(w \in \mathbb{\MakeUppercase{c}}^d\), this is similar we just do
	\[
		\max\{\left\vert v \cdot w \right\vert  \mid \left\lVert v\right\rVert_2 = 1\} = \left\lVert w\right\rVert_2.
	\]
	These maxes are achieved by \(v = \frac{\overline{w} }{\left\lVert w\right\rVert_w}\) if \(w \neq 0\).
\end{eg}
...
\begin{proof}
	...
	Thus, the supremum is \(\leq \left\lVert g\right\rVert_q\).

	\begin{enumerate}
		\item Let
		      \[
			      f(x) = \frac{\left\vert g(x)\overline{\sgn(g(x))} \right\vert ^{q-1}}{\left\lVert g\right\rVert_q^{q-1}}
		      \]
		      Then \(\int \left\vert f \right\vert ^p = 1\), and \(\int fg = \left\lVert g\right\rVert _q\).\todo{Check}
		\item DIY for handling the case when \(\mu\) is \hyperref[def:finite-measure]{\(\sigma\)-finite} and \(q = \infty, p = 1\).
	\end{enumerate}
\end{proof}
...
\begin{definition}[Dual space]\label{defn:dual-space}
...
\end{definition}
\begin{remark}
	Namely, \hyperref[def:bounded-linear-transformation]{BLTs} with codomain being the scalar field.
\end{remark}
...
\end{corollary}
\begin{proof}
	\(\ell_g\) is clear linear in \(f\) because the integral is linear. Then \autoref{prop:Lp-dual-formula} gives in both 1. and 2. that
	\[
