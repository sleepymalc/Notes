...
\end{remark}
\begin{explanation}
	Given \(\left(\mathbb{\MakeUppercase{N}}, \mathcal{P} (\mathbb{\MakeUppercase{N}}), \mu \right)\) where \(\mu\) is the \hyperref[eg:counting-measure]{counting measure}.
	Then we see
	\begin{itemize}
...
\end{note}
\begin{remark}
	From the property of \hyperref[def:measure]{measure}, the condition for a \hyperref[def:measure-space]{measure space} \((X, \mathcal{\MakeUppercase{a}} , \mu )\)
...
\begin{explanation}
	...
	non-negative. Finally, a \hyperref[def:mu-subnull-set]{\(\mu \)-subnull set} is always in \(\mathcal{\MakeUppercase{a}} \).
\end{explanation}
...
\begin{proposition}\label{prop:outer-measure}
	...
	\[
		\mu^{\ast} (A) \coloneqq \inf\left\{\sum\limits_{i=1}^{\infty} \rho(E_{i})\colon \underset{i\in \mathbb{\MakeUppercase{N}} }{\forall}E_{i}\in \mathcal{E},\ \bigcup\limits_{i=1}^{\infty} E_{i}\supset A\right\}
	\]
...
\begin{theorem}[Tonelli's Theorem for series]\label{thm:Tonelli-theorem-for-series}
	...
	\(a_{ij}\in [0, \infty ]\), \(\forall i, j\in \mathbb{\MakeUppercase{N}} \), then
	\[
