...
\begin{definition}[\(\mu\)-null, \(\mu\)-subnull, Complete measure space]
	...
	\begin{itemize}
		...
		\item\label{def:complete-measure-space} \((X, \mathcal{A} , \mu)\) is a \emph{complete} measure space if every \hyperref[def:mu-subnull-set]{\(\mu\)-subnull set} is \hyperref[def:A-measurable-set]{\(\mathcal{A}\)-measurable}.
	\end{itemize}
\end{definition}
\begin{note}
	We see that for a \hyperref[def:mu-subnull-set]{\(\mu\)-subnull set}, it's not necessary \hyperref[def:A-measurable-set]{\(\mathcal{A}\)-measurable}.
\end{note}
