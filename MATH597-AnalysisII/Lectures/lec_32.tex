...
\subsection{Lebesgue Differentiation Theorem for Regular Borel Measures}
...
\begin{proof}
	...
	\(\mathbb{R}^d = A \cup B\) disjoint with \(\lambda(A) = 0\), \(m(B) = 0\). Thus, it suffices to show \(m(G_n \cap A) = 0\).\footnote{Alternatively,
		we can simply define \(G_{n} \) over \(A\) instead of \(\mathbb{\MakeUppercase{r}} ^d\), as in Folland\cite{folland1999real}.}
