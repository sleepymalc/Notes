...
\begin{definition}[Total variation function]\label{def:total-variation-function}
	...
	\[
		T_F(x) = \sup \left\{\sum_{i=1}^n \left\vert F(x_i) - F(x_{i-1}) \right\vert \mid n \in \mathbb{N}, -\infty < x_0 < x_1 < \ldots < x_n = x\right\}.
	\]
\end{definition}
...
\begin{lemma}\label{lma:lec34-1}
	\(T_F(b)\) is equal to
	\[
		T_F(a) + \sup \left\{\sum_{i=1}^n \left\vert F(x_i) - F(x_{i-1}) \right\vert \mid n \in \mathbb{N}, a = x_0 < x_1 < \ldots < x_n = b\right\}
	\]
	if \(a < b\).
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
	Fix \(\epsilon > 0\) and for a \(x\in \mathbb{R} \), there exists \(x_0 < \ldots < x_n = x \) such that
	\[
