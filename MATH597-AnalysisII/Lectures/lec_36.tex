...
\subsection{Absolutely Continuous Functions}\label{subs:absolutely-cts}
We start with a definition.
\begin{definition}[Absolutely continuous]\label{def:absolutely-continuous}
...
\begin{lemma}
	We have that
	\begin{enumerate}[(1)]
		\item If \(F\) is \hyperref[def:absolutely-continuous]{absolutely continuous}, then it is uniformly continuous.
		\item If \(F\) is \hyperref[def:Lipschitz]{Lipschitz}, then \(F\) is \hyperref[def:absolutely-continuous]{absolutely continuous}.
		\item \(F(x) = \int_{-\infty}^x f(t) \,\mathrm{d}t\), \(f \in L^1\), is \hyperref[def:absolutely-continuous]{absolutely continuous}.
	\end{enumerate}
\end{lemma}
\begin{proof}
	...
	\begin{enumerate}[(1)]
		...
		\item We write this out as
		      ...
		      This directly implies that this function is \hyperref[def:absolutely-continuous]{absolutely continuous}.
	\end{enumerate}
\end{proof}

\begin{eg}
	The \hyperref[sssec:Cantor-Function]{Cantor function} \(F\) is uniformly continuous.
	However, we will see that it is not \hyperref[def:absolutely-continuous]{absolutely continuous}.
\end{eg}

\begin{proposition}\label{prop:abs-cts}
	Suppose \(F \in NBV\), then \(F\) is \hyperref[def:absolutely-continuous]{absolutely continuous} if and only if \(\mu_F \ll m\).
\end{proposition}
\begin{proof}
...
\begin{corollary}
	...
	\]
	where \(\mu_d\) is \hyperref[def:discrete-measure]{discrete}, \(\mu_{ac}\) is \hyperref[def:absolutely-continuous]{absolutely continuous},
