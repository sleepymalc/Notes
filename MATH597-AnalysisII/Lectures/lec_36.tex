...
\end{theorem}
\begin{proof}
	This follows directly from \autoref{}.\todo{Figure out the corresponding references.}
\end{proof}
...
\end{lemma}
\begin{proof}
	We see that \((2)\) and \((3)\) are obvious, so we prove \((1)\) only. Let \(E = \{x\in \mathbb{\MakeUppercase{r}} \mid \mu (\{x\}) = 0\}\), then
