...
We see that we can start with any collection of open sets \(\mathcal{E}\) and any \(\rho\) such that it assign measure on \(\mathcal{E}\), then induces an outer measure by \autoref{prop:outer-measure},
...
\begin{lemma}
	(1) + (2) \(\implies \) (3) in \autoref{thm:Caratheodory-extension-Thm}.
\end{lemma}
\begin{proof}
	\todo{DIY}
\end{proof}
...
\begin{proof}\let\qed\relax
	...
	\begin{itemize}
		\item We first show \(\mathcal{A} \supset \mathcal{A} _0\). Let \(A\in \mathcal{A} _0\), we want to show \(A\in \mathcal{A} \), i.e.,
		      ...
		      \[
			      \mu^{*} (E) + \epsilon \geq \sum\limits_{n=1}^{\infty} \mu_0(B_{n}) = \sum\limits_{n=1}^{\infty} \left(\mu_0(\underbrace{B_{n}\cap A}_{\in \mathcal{A} _0}) + \mu_0(\underbrace{B_{n}\cap A^{c}}_{\in \mathcal{A} _0} )\right)
		      \]
		      by the finite additivity of \(\mu_0\).
