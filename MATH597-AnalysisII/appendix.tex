...
\begin{proof}
	...
	\begin{enumerate}
		\item To show \(\mu _{F}(\{r\}) > 0\) for every \(r\in \mathbb{\MakeUppercase{q}} \), we first note that \(\{r\} = \bigcap\limits_{a-1\leq x< r}(x, r] \).
		      Then, we see that
		      \[
		      ...
		      assign the order of the index by the order of \(x\). Then, we see that \(\mu _{F}(r-1, r]<\infty \).\footnote{This will be \(\mu (A_1)\) in the condition of continuity from above. Furthermore, since \(\mathbb{\MakeUppercase{q}} \) is countable, hence \(F(x)<\infty \) is promised.}
		      Then, from continuity from above, we see that
		      \[
		...
		\item Now, we show \(\mu _{F}(\mathbb{\MakeUppercase{r}} \setminus \mathbb{\MakeUppercase{q}} ) = 0\). Firstly, we claim that
		      ...
		      \]
		      for any \(a\in\mathbb{\MakeUppercase{r}} \), say \(0\). From continuity from below, we have
		      \[
