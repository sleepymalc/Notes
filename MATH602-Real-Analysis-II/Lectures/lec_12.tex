...
\begin{proposition}[Baire category theorem]\label{prop:Baire-category}
	A complete metric space \(M\) is \textbf{never} the union of a countable number of \hyperref[def:nowhere-dense]{nowhere dense} sets.
\end{proposition}
\begin{proof}
...
\begin{proof}
	...
	\[
		\overline{TB_X}\supseteq \epsilon B_Y
	\]
	for some \(\epsilon > 0\). Finally, we show that \(\overline{TB_X} \subseteq T(2 B_X)\), which will imply
	\[
...
\end{proposition}
\begin{proof}
	To show that (a) implies (b), we see that \(T^{-1} \colon \im(T) \to X\) is \hyperref[rmk:bounded-op]{bounded}  since \(\im(T)\) is \hyperref[def:Banach-space]{Banach space}, from \autoref{thm:open-mapping},
	\[
...
\begin{proposition}[Hellinger-Toeplitz theorem]\label{thm:Hellinger-Toeplitz}
	Let \(T\colon \mathcal{\MakeUppercase{h}} \to \mathcal{\MakeUppercase{h}} \) be a \hyperref[def:linear-op]{linear operator} which is self-\hyperref[def:adjoint-op]{adjoint}. Then if \(\left\langle Tx, y \right\rangle  = \left\langle x, Ty \right\rangle\) for \(x, y\in \mathcal{\MakeUppercase{h}} \), \(T\) is \hyperref[rmk:bounded-op]{bounded}.
\end{proposition}
\begin{proof}
