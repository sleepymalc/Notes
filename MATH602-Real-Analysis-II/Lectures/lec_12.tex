...
Finally, as we know, the \hyperref[def:kernel]{kernel} of every \hyperref[def:bounded-linear-op]{bounded linear operators} \(T\in \mathcal{L} (X, Y)\) is always a closed subspace, while the \hyperref[def:image]{image} of \(T\) may or may not be closed. We can also characterize this.

\begin{proposition}[Isomorphic embedding]\label{prop:isomorphic-embedding}
...
\begin{theorem}[Hellinger-Toeplitz theorem]\label{thm:Hellinger-Toeplitz}
	Let \(T\colon \mathcal{H} \to \mathcal{H} \) be a \hyperref[def:linear-op]{linear operator}. If \(T\) is \hyperref[def:self-adjoint-op]{self-adjoint}, i.e., \(\left\langle Tx, y \right\rangle  = \left\langle x, Ty \right\rangle\) for \(x, y\in \mathcal{H} \), then \(T\) is \hyperref[rmk:bounded-op]{bounded}.
\end{theorem}
\begin{proof}
