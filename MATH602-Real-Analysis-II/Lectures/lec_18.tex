...
\end{theorem}
\begin{proof}
	Without loss of generality, we prove that \(T^{\ast} B_{Y^{\ast} }\) is \hyperref[def:precompact]{precompact} in \(X^{\ast}\) given \(T\in K(X, Y)\), i.e., consider \(f_n \in B_{Y^{\ast} }\) for \(n \geq 1\) such that \(\{T^{\ast} f_n\}_{n\geq 1}\) has a convergent subsequence.
