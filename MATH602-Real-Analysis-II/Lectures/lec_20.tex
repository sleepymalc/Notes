...
\begin{eg}[Shift operator]
	Consider the shift operators on \(\ell _2 = \left\{ (a_1, a_2, \dots ), a_j\in \mathbb{R}, \sum_{j=1}^{\infty}  a_j^2 < \infty \right\} \). The right shift \(T_r\) is defined as
	\[
		T_r(a_1, a_2, \dots) = (0, a_1, a_2, \dots ),
	\]
	which is injective but not surjective; while the left shift \(T_{\ell} \) is defined as
	\[
		T_{\ell } (a_1, a_2, \dots ) = (a_2, a_3, \dots ),
	\]
