...
\]
for \(x\in \mathcal{H} \) where \(E_{n} = \operatorname{span}(\left\{ x_1, \dots , x_{n}\right\})\). We see that \(S_n\) is a \hyperref[def:linear-map]{linear operator} and \(S_n= P_{E_n}\) is the \hyperref[def:orthogonal-projection]{orthogonal projection} onto \(E_n\) since \(\left\langle x -S_n(x), x_k \right\rangle = 0\) for \(k = 1, \dots  , n\), hence \(S_{n} (x)\in E_{n}\) and \(x-S_n(x)\perp E_n\).
