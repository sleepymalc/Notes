...
\begin{explanation}
	...
	\begin{intuition}
		...
		\[
			\left\{ F\in \mathcal{\MakeUppercase{b}} \colon \colon \int _F f\,\mathrm{d} t = 0 \right\},
		\]
...
\begin{explanation}
	...
	\[
		\left\vert f(\frac{x_{n} }{n \left\lVert x_{n} \right\rVert }) \right\vert \geq 1, \quad n = 1, 2, \ldots.
	\]
...
\begin{proposition}
	...
	\begin{enumerate}[(a)]
		\item For every \hyperref[def:linear-functional]{linear functional} on \(E\), \(\ker(f)\) is a \hyperref[def:hyperplane]{hyperplane} in \(E\).
		\item If \(f, g \neq 0\) are \hyperref[def:linear-functional]{linear functionals} on \(E\) such that \(\ker(f) = \ker(g)\), then \(f = ag\) for some \(a \neq 0\).
		\item For every \hyperref[def:hyperplane]{hyperplane} \(H\subseteq E\), there exists a \hyperref[def:linear-functional]{linear functional} \(f \neq 0\) on \(E\) such that \(\ker(f) = H\).
		\item If \(E\) is a \hyperref[def:Banach-space]{Banach space}, then \(f(\cdot)\) is bounded if and only if \(\ker(f) = H\) is closed.
	\end{enumerate}
\end{proposition}
\begin{proof}
