...
Suppose \(x_1, x_2, \dots \in \mathcal{H}\) is a set of vectors and \(E_n = \operatorname{span}(\left\{ x_1, \dots , x_n \right\} )\). Then we can find an \hyperref[def:orthonormal-system]{orthonormal set} \(\left\{ y_{k} \right\}_{k\geq 1}\) in \(\mathcal{H}\) such that \(E_n = \operatorname{span}(\left\{ y_1, y_2, \dots , y_{m(n)}  \right\} )\) where \(m(n) \leq n\). Such a procedure is called \emph{Gram-Schmidt orthogonalization}. To do this, firstly, set \(y_1 = x_1 / \left\lVert x_1\right\rVert \), then for \(n \geq 2\), we have
\[
...
Note that we can effectively compute the vectors \(P_{E_n} (x_{n+1}) \) since we know that \(S_n(x)\) is the \hyperref[def:orthogonal-projection]{orthogonal projection} of \(x\) onto \(\operatorname{span}(\left\{ y_k \right\}_{k\geq 1} )\), which is the partial sum of \hyperref[def:Fourier-series]{Fourier series}
\[
