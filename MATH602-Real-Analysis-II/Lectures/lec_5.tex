...
Suppose \(x_1, x_2, \ldots \in \mathcal{H}\) is a set of vectors and \(E_n = \mathop{\mathrm{span}}(\left\{ x_1, \ldots , x_n \right\} )\). Then we can find an \hyperref[def:orthonormal-system]{orthonormal set} \(\left\{ y_{k} \right\}_{k\geq 1}\) in \(\mathcal{H}\) such that \(E_n = \mathop{\mathrm{span}}(\left\{ y_1, y_2, \ldots , y_{m(n)}  \right\} )\) where \(m(n) \leq n\). Such a procedure is called \emph{Gram-Schmidt orthogonalization}. To do this, firstly, set \(y_1 = x_1 / \left\lVert x_1\right\rVert \), then for \(n \geq 2\), we have
\[
