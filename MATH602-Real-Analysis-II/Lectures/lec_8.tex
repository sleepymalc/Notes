...
We now see the proof of \hyperref[thm:Hahn-Banach]{Hahn-banach theorem}.
...
Before we end this section, we see some corollaries of \hyperref[thm:Hahn-Banach]{Hahn-Banach theorem}. From \autoref{thm:supporting-functional}, we see that for every vector \(x\), we indeed attain its \hyperref[def:norm]{norm} on some \hyperref[def:linear-functional]{functional} \(f\in E^{\ast} \), i.e., their supporting \hyperref[def:linear-functional]{functional}. But recall that the \hyperref[def:norm]{norm} of a \hyperref[def:linear-functional]{functional} \(f\in E^{\ast} \) is defined as
\[
	\left\lVert f\right\rVert \coloneqq \sup _{x \neq 0}\frac{\left\vert f(x) \right\vert }{\left\lVert x\right\rVert },
\]
and in general, \(f\) will not attain its \hyperref[def:norm]{norm} on some vector \(x\). Above observation leads to the following.

\begin{corollary}
	For every vector \(x\) in a \hyperref[def:normed-vector-space]{normed space} \(E\),
	\[
		\left\lVert x\right\rVert = \max _{f \neq }\frac{\left\vert f(x) \right\vert }{\left\lVert f\right\rVert }
	\]
	where the maximum is taken over all non-zero \hyperref[def:linear-functional]{functionals} \(f\in E^{\ast} \).
\end{corollary}
...
\]
i.e., we have a \emph{canonical embedding} of \(E\) into \(E^{\ast\ast}\). The above discussion leads to \autoref{thm:second-dual-space}.
...
\begin{definition}[Reflexive space]\label{def:reflexive-space}
	A \hyperref[def:normed-vector-space]{normed space} \(E\) is called \emph{reflexive space} if \(E \to E^{\ast\ast} \).\footnote{Under the canonical embedding.}
\end{definition}


\begin{eg}
	\(L_p\) spaces for \(1 < p < \infty \) are \hyperref[def:reflexive-space]{reflexive spaces}.
\end{eg}
\begin{explanation}
	We know that \(L_p^{\ast} = L_q\) where \(1 \leq p < \infty \) for \(q\) being the conjugate index of \(p\).
\end{explanation}
\begin{eg}
	\(L_p\) spaces for \(p = 1\) or \(\infty \) are not \hyperref[def:reflexive-space]{reflexive spaces}
\end{eg}
...
\begin{remark}[James' theorem]
	The converse of \autoref{prop:lec8} is also true, i.e., if every \hyperref[def:linear-functional]{functional} \(f\in E^{\ast} \) on a \hyperref[def:Banach-space]{Banach space} \(E\) attains its \hyperref[def:norm]{norm}, then \(E\) is are \hyperref[def:reflexive-space]{reflexive}. This is the so-called \href{https://en.wikipedia.org/wiki/James%27s_theorem}{\emph{James' theorem}}.
\end{remark}