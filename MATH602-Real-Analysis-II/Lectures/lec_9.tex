...
\begin{theorem}[Separation of a point from a convex set]\label{thm:separation-of-a-point-from-a-convex-set}
	Let \(K\) be an open convex subset of a normed space \(E\) and \(x_0 \notin K\). Then there exists a continuous linear functional \(f\colon E\to \mathbb{\MakeUppercase{r}} \) with \(f\equiv 0\) and \(f(x) < f(x_0)\) for \(x\in K\).
\end{theorem}
\begin{proof}
	...
	\]
	for \(x\in E\) if \(B(0, r)\subseteq K\). Proceed as in \autoref{thm:supporting-functional} for unit ball, we define \(f_0\) on \(\mathop{\mathrm{span}}(x_0)\) by
	\[
		f_0(tx_0) = t\left\lVert x_0\right\rVert
	\]
	for \(t\in \mathbb{\MakeUppercase{r}} \). Then if \(E_0 = \left\{ \lambda x_0 \colon \lambda \in \mathbb{\MakeUppercase{r}}  \right\} \), \(f_0 (x) \leq \left\lVert x\right\rVert _K\) for \(x\in E_0\) since \(\left\lVert tx\right\rVert _K = t\left\lVert x\right\rVert _K\) for \(t \geq 0\). By \autoref{thm:Hahn-Banach}, we can extend \(f_0\) to \(f\colon E\to \mathbb{\MakeUppercase{r}} \) such that
	\[
		f(x) \leq \left\lVert x\right\rVert _K \leq \frac{1}{r} \left\lVert x\right\rVert
	\]
	for \(x\in E\), hence \(f\) is a bounded linear functional. For separation, we see that
	\[
		f(x) \leq \left\lVert x\right\rVert _K \leq 1 \leq \left\lVert x_0\right\rVert _K = f_0 (x_0) = f(x_0)
	\]
	for \(x\in K\), hence \(f(x) \leq f(x_0)\). Now, since \(K\) is open, so \(x + tv\in K\) for some \(t > 0\) and all \(v\) with \(\left\lVert v\right\rVert = 1\). Hence, we have
	\[
		f(x + tv) \leq f(x_0)
	\]
	for \(t = t_x > 0\), \(\left\lVert x\right\rVert = 1\), which further implies
	\[
		f(x) + t \sup _{\left\lVert x\right\rVert = 1}f(x) \leq f(x_0) \implies \sup _{\left\lVert x\right\rVert = 1} > 0 \implies f(x) < f(x_0).
	\]
\end{proof}
...
\begin{theorem}[Separation of convex sets]\label{thm:separation-of-convex-sets}
	...
	\begin{enumerate}[(a)]
		\item If \(A\) is open, then there exists a bounded linear functional \(f\colon E\to \mathbb{\MakeUppercase{r}} \) such that \(f(a) < f(b)\) for \(a\in A\), \(b\in B\).
		\item If \(A\), \(B\) are closed and \(B\) is compact, then \(\sup _{a\in A} f(a) < \inf _{b\in B}f(b)\).
	\end{enumerate}
\end{theorem}
\begin{proof}
	We have the following.
	\begin{enumerate}[(a)]
		\item Let \(K = A - B = \left\{ a - b\colon a\in A, b\in B \right\} \). Then \(K\) is open, \hyperref[def:convex-set]{convex} and \(0 \notin K\). By \autoref{thm:separation-of-a-point-from-a-convex-set}, there exists \(f\in E^{\ast} \) such that
		      \[
			      f(a - b) < f(0) = 0
		      \]
