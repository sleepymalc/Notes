...
\begin{theorem}[Fourier Series]\label{thm:fourier-series}
	The set of \(e_n(x) = \frac{1}{\sqrt{2\pi} }e^{inx}\) for \(n \in \mathbb{\MakeUppercase{z}} \) is an
...
\begin{proof}
	...
	\(N \in \mathbb{\MakeUppercase{n}} \) such that \(\left\lVert g_{r,N} - g_r\right\rVert _\infty < \frac{\epsilon }{3\sqrt{2\pi}}\),
...
\begin{eg}[Isoperimetric inequality.]
	Consider a parametric curve \((x(t),y(t)) \in \mathbb{\MakeUppercase{r}}^2\) with \(t \in [-\pi,\pi]\). Assume that
	\begin{enumerate}[(1)]
