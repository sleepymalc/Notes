...
\begin{explanation}
	...
	\]
	Observe that the \hyperref[def:cogeodesic-flow]{cogeodesic flow} maps the set
	\[
...
\begin{remark}
	If \(\mathcal{M} \) is compact, then all \(E_\lambda \) are compact, then all \hyperref[def:geodesic-flow]{geodesic flow} is defined on all of \(E_\lambda \) for all \(\lambda \).
\end{remark}
...
With these tools, we now introduce the \hyperref[def:vector-field-along-curve]{vector field along a curve}.
...
We can now finally define \hyperref[def:covariant-derivative]{covariant derivative} formally, and see why we can use them interchangeably with \hyperref[def:linear-connection]{linear connection}.
...
\end{proposition}
\begin{proof}
	The \hyperref[def:covariant-derivative]{covariant derivative} along \(c\) is defined by
	\[
...
\begin{remark}[Geometric significant of Riemannian curvature]
	The idea is that for a \hyperref[def:smooth-manifold]{manifold} with \hyperref[def:torsion-free]{torsion free} \(\nabla \), if we \hyperref[def:parallel-transport]{parallel transporting} along two paths on an infinitesimal patch (which induces \(X, Y\)) such that \([X, Y]=0\), we can detect \hyperref[def:Riemannian-curvature]{curvature} in terms of \(\delta z\), where \((\delta z)^m = \ldots = R^m_{nab} X^a Y^b Z^n\cdot \delta s \delta t + O(\delta s^2 \delta t, \delta s \delta t^2)\).\footnote{This is a deep theorem! In the \(\ldots \), we use \(T \equiv 0\).}
	\begin{center}
...
\begin{intuition}
	An \hyperref[def:autoparallel]{autoparallel} \hyperref[def:curve]{curve} is the \emph{straightest line} in the space w.r.t.\ \(\nabla \), hence it gives us a \hyperref[def:geodesic]{geodesic}!
\end{intuition}
