\lecture{19}{14 Mar. 14:30}{Length-Minimizing Geodesics and Conjugacy}
Now, let's prove \autoref{thm:lec18-2}.
\begin{proof}[Proof of \autoref{thm:lec18-2}]
	We prove them one by one.
	\begin{enumerate}[(a)]
		\item First steps: last lecture.\todo{Add this} Now, cover \(\left\{ tV \mid 0 \leq t \leq 1 \right\} \) by finitely many such neighborhoods \(\Omega _i\), \(i = 1, \ldots , k\), and let \(U_i = \exp _p \Omega _i\). Assume that \(tV\in \Omega _i\), for \(t_{i-1} \leq t \leq t_i\) (with \(t_0 = 0, t_k = 1\)). Let \(\epsilon > 0\) sufficiently small. Then, for all \hyperref[def:curve]{curve} \(g\colon [0, 1] \to \mathcal{M} \) satisfying the assumption, \(g([t_{i-1}, t_i]) \subseteq U_i\).
