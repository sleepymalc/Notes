...
Let's resume our proof about \autoref{lma:conjugate-point-index-form}.
\begin{proof}[Proof of \autoref{lma:conjugate-point-index-form} (Continue)]
	For the backward direction, assume that for \(t_0, t_1\in [a, b]\) (without loss of generality, let \(t_0 < t_1\)) such that \(c(t_0), c(t_1)\) are two \hyperref[def:conjugate-point]{conjugate points} along \(c\). Then, there exists a non-trivial \hyperref[def:Jacobi-field]{Jacobi field} \(X\) along \(c\) such that \(X(t_0) = 0 = X(t_1)\). Fix this, and consider
	\[
		Y(t) = \begin{dcases}
