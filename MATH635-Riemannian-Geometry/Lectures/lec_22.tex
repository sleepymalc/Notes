...
\begin{definition}[Diameter]\label{def:diameter}
	...
	\[
		\diam(\mathcal{M} ) \coloneqq \sup _{p, q\in \mathcal{M} } d(p, q).
	\]
\end{definition}
...
\begin{theorem}[Bonnet-Mayers theorem]\label{thm:Bonnet-Mayers}
	...
	\[
		\Ric(X, X) \geq \lambda \left\langle X, X \right\rangle
	\]
...
\begin{proof}
	...
	\]
	Since this is true for all \(\rho < \diam(\mathcal{M} ) \), hence we see that \(\diam(\mathcal{M} ) \leq \pi \sqrt{(n-1) / \lambda } \).
...
\begin{intuition}
	\hyperref[thm:Bonnet-Mayers]{Bonnet-Mayers theorem} says that if \(\mathcal{M} \) has \hyperref[def:Ricci-curvature]{Ricci curvature} not less than the one of \(S^n_r\), then \(\diam(\mathcal{M} ) \) is at most the one of \(S^n_r\).
\end{intuition}
