...
We can now formally define the \hyperref[def:tangent-space]{tangent space} on a \hyperref[def:smooth-manifold]{smooth manifold}.

\begin{definition*}
	Let \(\mathcal{M} \) be a \hyperref[def:smooth-manifold]{differentiable manifold} of dimension \(d\), \(x\colon U \subseteq \mathcal{M} \to  \mathbb{R} ^d\) be a \hyperref[def:coordinate-chart]{chart}, and \(p\in U \subseteq \mathcal{M} \) where \(U\) is open.

	\begin{definition}[Tangent space]\label{def:tangent-space}
		The \emph{tangent space \(T_p \mathcal{M} \) of \(\mathcal{M} \) at \(p\)} is represented in the \hyperref[def:coordinate-chart]{chart} \(x\) by \(T_{x(p)}x(U)\).
	\end{definition}
...
\[
	\left( \frac{\partial F^\alpha }{\partial x^i}  \right) _{\substack{\alpha =1, \ldots , c \\ i = 1, \ldots , d}},
\]
