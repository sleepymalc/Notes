...
Let's first introduce the theory of \hyperref[def:bundle]{bundles}, which allows us to introduce the notion of \hyperref[def:vector-field]{vector fields}. The motivation is clear: we have already seen what a \hyperref[def:vector-field]{tangent vector field} \(X\) of a \hyperref[def:smooth-manifold]{smooth manifold} \(\mathcal{M} \) is: we simply associate \(X(p)\) to a \hyperref[def:tangent-vector]{tangent vector}:
...
% https://q.uiver.app/?q=WzAsMixbMCwwLCJFIl0sWzAsMSwiXFxtYXRoY2Fse019Il0sWzAsMSwiXFxwaSIsMl0sWzEsMCwicCIsMix7ImN1cnZlIjoxfV1d
\[
	\begin{tikzcd}
		E \\
		{\mathcal{M}}
		\arrow["\pi"', from=1-1, to=2-1]
		\arrow["p"', curve={height=6pt}, from=2-1, to=1-1]
	\end{tikzcd}
\]
...
Finally, introduce the notion of \hyperref[def:vector-field]{vector fields} in terms of \hyperref[def:section]{section}.
...
\begin{notation}
	For the following discussion, since we will nearly always be talking about \hyperref[def:vector-field]{tangent vector fields}, we will abuse the notation a bit and just simply call it \hyperref[def:vector-field]{vector fields} in our context. But always keep in mind that more broadly, a \hyperref[def:vector-field*]{vector field} should be a \hyperref[def:section]{section} of a \hyperref[def:bundle]{bundle}, not always \(T \mathcal{M} \).
\end{notation}