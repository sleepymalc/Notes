...
\begin{proof}
	...
	\end{itemize}
	If \(\ACF \models \varphi \), then we will halt at the first case. Now suppose \(\ACF \not \models \varphi \), i.e., there is \(\mathcal{M} \models \ACF \) such that \(\mathcal{M} \models \lnot \varphi \), and also, there is \(p\) such that \(\mathcal{M} \models \ACF_p\). Since \(\ACF_p\) is \hyperref[def:theory-complete]{complete}, \(\ACF_p \models \lnot \varphi \), so the search of the second case will half, hence the whole search will eventually halt.
\end{proof}