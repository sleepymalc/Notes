...
In \hyperref[def:algebraically-closed]{algebraically closed} fields, \(a\in \acl(X) \) just means that \(a\) is \hyperref[def:algebraic]{algebraic} over (the field generated by) \(X\) , i.e., there is \(p(y) \in F_X[\overline{x} ]\) such that \(p(a) = 0\). In vector spaces, \(a\in \acl(X) \) if and only if \(a\in \operatorname{span}(X) \).
