...
\begin{theorem}[Weak Hilbert's Nullstellensatz]\label{thm:weak-Hilbert-Nullstellensatz}
	Let \(K\) be \hyperref[def:algebraically-closed]{algebraically closed}, and \(f_1, \dots , f_n\in K[\overline{x} ]\). Then there is \(\overline{a} \in K^m\) such that \(f_1(\overline{x} ) = \dots = f_n(\overline{x} ) = 0\) if and only if \(1 \notin (f_1, \dots , f_n)\), i.e., there are no \(r_1, \dots , r_n\in K[\overline{x} ]\) such that \(1 = r_1 f_1 + \dots + r_n f_n\).
\end{theorem}
\begin{proof}
