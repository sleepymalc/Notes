...
Our two examples of \hyperref[def:countably-categorical]{countably categorical} \hyperref[def:theory]{theories}, \(\DLO\) and the \hyperref[def:random-graph-theory]{random graph}, are both characterized by an \hyperref[int:extension-axiom]{extension axiom}.\footnote{Although we didn't mention this explicitly for \(\DLO\), but essentially given \(a<b\), we can ``extend'' this order by finding \(c\) such that \(a < c < b\).} This influences a general way of construction \hyperref[def:countably-categorical]{countably categorical} \hyperref[def:structure]{structures}.

\section{Substructures' Properties}

\begin{definition}[Generated substructure]\label{def:generated-substructure}
	Given an \hyperref[def:structure]{\(\mathcal{L} \)-structure} \(\mathcal{M} \), and a set \(A \subseteq M\), we write \(\left\langle A \right\rangle \) for the \hyperref[def:substructure]{substructure} of \(M\) \emph{generated} by \(A\).
\end{definition}

\begin{remark}
	Compare \autoref{def:generated-substructure} and the \hyperref[not:generated-substructure]{substructure generated} in the \hyperref[thm:downward-Lowenheim-Skolem]{downward Löwenheim-Skolem theorem}.
\end{remark}

\begin{intuition}
	This is the smallest \hyperref[def:substructure]{substructure} of \(\mathcal{M} \) whose domain contains \(A\).\footnote{Equivalently, it's the \hyperref[def:substructure]{substructure} of \(\mathcal{M} \) which contains \(A\) and all the constant, and is closed under the application of functions.}
\end{intuition}
