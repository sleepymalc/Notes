\lecture{22}{28 Mar.\ 11:30}{Ryll-Nardzewski Theorem}
\begin{eg}
...
\begin{eg}
	\(\text{random \(K_d\) graph} = \Flm(\text{finite \(K_d\)-free graphs}) \).
\end{eg}
...
\begin{eg}
	\((\mathbb{N} ^2, \sim ) = \Flm(\text{finite sets equipped with \(\sim \)})\) where \((a, b)\sim (c, d) \iff a = c\).
\end{eg}

\begin{theorem}\label{thm:lec22}
	Let \(\mathcal{L} \) be a finite relational \hyperref[def:language]{language}. Then every \hyperref[def:ultrahomogeneous]{ultrahomogeneous} \hyperref[def:structure]{\(\mathcal{L} \)-structure} \(\Th(\mathcal{M})\) (i.e., \(\mathcal{M} = \Flm(\Age(\mathcal{M} )) \)) is \hyperref[def:countably-categorical]{countably categorical} and has \hyperref[def:quantifier-elimination]{quantifier elimination}.
\end{theorem}
\begin{proof}
...
\begin{eg}
	\(F^{\oplus w} = \Flm(\text{finite dimensional \(F\)-vector spaces} ) \) for all countable field \(F\).
\end{eg}

\begin{eg}
	\(\mathbb{F} _p = \Flm(\text{finite \hyperref[def:characteristic]{characteristic} \(p\) fields})\) for prime \(p\).
\end{eg}