\lecture{9}{2 Feb. 14:30}{Proving the \hyperref[thm:completeness]{Completeness Theorem}}
...
\begin{proof}\let\qed\relax
	The \hyperref[def:model]{model} we build is called a \underline{canonical model}. Let \(\mathcal{C} \) be the set of constants in \(\mathcal{L} \), and define an equivalence relation \(\sim \) on \(\mathcal{C} \) by \(c \sim d\) if and only if \(c=d\) is in \(T\).
	...
	\begin{enumerate}[(a)]
		\item \(c^{\mathcal{M} } = [c]\).
		\item \(R^{\mathcal{M} }([c_1], \ldots , [c_n]) \) true if \(R(c_1, \ldots , c_n)\) is in \(T\). This is well-defined by \autoref{A7} \autoref{def:axioms-for-equality-e}.
		\item \(f^{\mathcal{M} }([c_1], \ldots , [c_n]) = [d]\) if \(f(c_1, \ldots , c_n) = d\) is in \(T\). Such a \(d\) exists because \(\exists x\ f(c_1, \ldots , c_n) = x\), i.e., \(\lnot \forall x\ f(c_1, \ldots , c_n) \neq x\), is in \(T\).\footnote{Otherwise, \(\forall x\ f(c_1, \ldots , c_n) \neq x\) is in \(T\). By \autoref{A4}, \(f(c_1, \ldots , c_n) \neq f(c_1, \ldots , c_n)\) is in \(T\), contradicts to \autoref{A7} \autoref{def:axioms-for-equality-a}.} If this is in \(T\), then there is a \hyperref[def:Henkin-constant]{Henkin constant} \(d\) with \(f(c_1, \ldots , c_n) = d\) in \(T\). To show that this is well-defined, from \autoref{A7} \autoref{def:axioms-for-equality-d}, i.e.,
		      \[
			      (t_1 = u_1 \land \ldots \land t_n = u_n) \to f(t_1, \ldots , t_n) = f(u_1, \ldots , u_n).
		      \]
		      So if \([c_1] = [d_1], \ldots , [c_n] = [d_n]\), then \(c_1 = d_1, \ldots , c_n = d_n\) are in \(T\). So \(f(c_1, \ldots , c_n) = f(d_1, \ldots , d_n)\) is in \(T\) by \autoref{A7} \autoref{def:axioms-for-equality-d}. If \(a\) and \(b\) are constants such that \(f(c_1, \ldots , c_n) = a\) and \(f(d_1, \ldots , d_n) = b\) are in \(T\), so \(a = b\) is in \(T\) by \autoref{A7} \autoref{def:axioms-for-equality-c}, i.e., the transitivity of \(=\).
	\end{enumerate}
	...
	\begin{enumerate}
		\item \hyperref[def:term]{Terms}: Show that \(t^{\mathcal{M} }([c_1], \ldots , [c_n]) = [d]\) if and only if \(t(c_1, \ldots , c_n) = d\) is in \(T\).
		      \begin{enumerate}
			      ...
			      \item Suppose that \(t(x_1, \ldots , x_n) = f(s_i(x_1, \ldots , x_n), \ldots , s_m(x_1, \ldots , x_n))\). Let
			            ...
			            \]
			            by the inductive hypothesis, \(d_i = s_i(c_1, \ldots , c_n)\) is in \(T\). Let \([e] = f^{\mathcal{M} }([d_1], \ldots , [d_m]) = t^{\mathcal{M} } ([c_1], \ldots , [c_n])\). By the definition of \(f\), \(e = f(d_1, \ldots , d_m)\) is in \(T\). By \autoref{A7} \autoref{def:axioms-for-equality-d},
			            \[
				            e = f(s_1(c_1, \ldots , c_n), \ldots , s_m(c_1, \ldots , c_n))
			            \]
			            is in \(T\). This is the direction (\(\implies \)).

			            Now suppose that \(t(c_1, \ldots , c_n) = e^\prime \) is in \(T\). We want to show that \([e]=[e^\prime ]\), i.e., \(e = e^\prime \) is in \(T\). Since \(e = t(c_1, \ldots , c_n)\) is in \(T\), and \(e^\prime = t(c_1, \ldots , c_n)\) is in \(T\). By \autoref{A7} \autoref{def:axioms-for-equality-c}, \(e = e^\prime \) is in \(T\), so \([e^\prime ] = [e]\ = t^{\mathcal{M} }([c_1], \ldots , [c_n]) \). This is (\(\impliedby\)).
		      \end{enumerate}
		\item \hyperref[def:formula]{Formulas}: Show that \(\mathcal{M} \models \varphi ([c_1], \ldots , [c_n])\) if and only if \(\varphi (c_1, \ldots , c_n)\) is in \(T\).\footnote{In particular, for a \hyperref[def:sentence]{sentence} \(\varphi \), \(\mathcal{M} \models \varphi \iff \varphi \) is in \(T\), and so \(\mathcal{M} \models T\).}
		      \begin{enumerate}
			      \item If \(\varphi \) is \(s(x_1, \ldots , x_n)=t(x_1, \ldots , x_n)\):
			            \begin{enumerate}
				            \item[(\(\implies\))] If \(\mathcal{M} \models s([c_1], \ldots , [c_n]) = t([c_1], \ldots , [c_n])\),
					            ...
					            \]
					            Let \([d]\) be this element equal to the above, so \(d=s(c_1, \ldots , c_n)\) and \(d=t(c_1, \ldots , c_n)\) are in \(T\) so \(\underbrace{s(c_1, \ldots , c_n) = t(c_1, \ldots , c_n)}_{\varphi (c_1, \ldots , c_n)}\) is in \(T\) by \autoref{A7} \autoref{def:axioms-for-equality-c}.
