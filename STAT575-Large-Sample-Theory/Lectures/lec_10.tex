\lecture{10}{15 Feb.\ 9:30}{Fundamental Theorems of Probability}
\section{Fundamental Theorems of Probability}
With the tools we developed, we now prove two of the fundamental theorems of probability, i.e., the \hyperref[thm:WLLN]{weak law of large number}, and also the \hyperref[thm:CLT]{central limit theorem}. We start by the first one.
...
\begin{answer}
	...
	\[
		...
		\end{split}
	\]
	As \(n \to \infty \), \(\hat{\sigma} _n^2 / \sqrt{n} \overset{p}{\to } 0\), and from \hyperref[thm:CLT]{central limit theorem}, if \(\mathbb{E}_{}[X^4] < \infty \),
	\[
	...
	\]
	hence we need the \emph{Kyrtosis} of \(X\), i.e., \(\mathbb{E}_{}[( (X-\mu ) / \sigma )^4] \) is \(3\), then we have what we derived before. Let \(Z = (X-\mu ) / \sigma \). Then we need \(\mathbb{E}_{}[Z^4] \geq \left( \mathbb{E}_{}[Z^2] \right) ^2 \geq 1\), hence the above makes sense when the equality doesn't hold. We see that the equality holds when \(Z^2 = 1\), i.e., \(Z \in \{ \pm 1 \} \).
\end{answer}