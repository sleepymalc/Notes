...
We will address two questions: what if \(X_i\)'s are not Gaussian, and can we replace \(s_n\) by \(\hat{\sigma} _n\).
...
\subsection{Inference for Population Variance}
...
The quantity in our assumption deserves a special name.
...
The takeaway is that if the \hyperref[def:Kurtosis]{Kurtosis} of \(X\) is different from the normal, then the distribution of \(\sqrt{n} (\hat{\sigma} _n^2 - \sigma ^2)\) is different. Moreover, we see that we can't quite do the same trick as before by making the distribution free from parameters to get a confident interval.
