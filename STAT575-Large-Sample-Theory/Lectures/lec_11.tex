\lecture{11}{20 Feb.\ 9:30}{Normality and Sample Standardized Central Moments}
Following the intuition, let's try to generalize what we have done and find such \hyperref[def:consistent]{consistent} estimators. Let \(Y \coloneqq X - \mu = X - \mathbb{E}_{}[X] \) (and also \(Y_i = X_i - \mu \) as usual), \(\mu _k \coloneqq \mathbb{E}_{}[Y^k] = \mathbb{E}_{}[(X - \mu )^k] \) for all \(k \geq 2\), and finally \(\widetilde{\mu} _k = \mu _k / \sigma ^k = \mathbb{E}_{}\left[ (X-\mu )^k / \sigma ^k \right] \).
...
\begin{proof}
	...
	\]
	By the \hyperref[thm:WLLN]{weak law of large number}, \(\overline{Y^k} \overset{p}{\to} \mathbb{E}_{}[Y^k] = \mu _k\) and \((-\overline{Y} )^{k-\ell } \overset{p}{\to} 0\) since \(-\overline{Y} \overset{p}{\to} 0\) with \hyperref[thm:continuous-mapping]{continuous mapping theorem}, hence \(M_k \overset{p}{\to} \mu _k\) by \hyperref[col:Slutsky]{Slutsky's theorem}. The \hyperref[def:consistent]{consistency} of \(\hat{\sigma} _n\) implies \(\widetilde{M} _k \overset{p}{\to} \widetilde{\mu} _k\) clearly.
\end{proof}

\subsection{Asymptotic Distribution of Sample Central Moments}
Furthermore, we can also ask for the asymptotic distribution of \(M_k\), i.e., \(\sqrt{n} (M_k - \mu _k)\). Consider our notations used so far.

\begin{theorem}\label{thm:asymptotic-distribution-sample-central-moment}
	If \(X \in L^{2k}\) for some \(k > 2\), then
	\[
		\sqrt{n} (M_k - \mu _k)
		= \frac{1}{\sqrt{n} } \sum_{i=1}^{n} (Y_i^k - \mu _k - k \mu _{k-1} Y_i) + o_p(1).
	\]
	Hence, if \(v_k \coloneqq \Var_{}[Y^k - k \mu _{k-1}Y] > 0\), then \(\sqrt{n} (M_k - \mu _k) \overset{D}{\to} \mathcal{N} (0, v_k)\).
\end{theorem}
\begin{proof}
	Firstly,
	\[
	...
	\end{itemize}
	Combining, every term in the summation is \(O(1) O_p(1) o(1) = o_p(1)\) except for \(\ell = k-1\), hence
	\begin{align*}
		\sqrt{n} (M_k - \mu _k)
	...
	\end{align*}
	Observe that \(Y_i^k - \mu _k - k \mu _{k-1} Y_i\)'s are i.i.d., the whole thing converges to \(\mathcal{N} (0, \Var_{}\left[Y^k - \mu _k - k \mu _k Y\right] )\) \hyperref[def:converge-in-distribution]{in distribution} by \hyperref[thm:CLT]{central limit theorem}, where the variance can be further simplified as
	\[
		\begin{split}
...
Note that \autoref{thm:asymptotic-distribution-sample-central-moment} doesn't give a confidence interval for estimating \(\sigma ^2\), which requires we to obtain the joint distribution of \(\hat{\sigma} _n^2\) and \(M_k\), and we only have separated ones.

\subsection{Testing Normality with Odd Moments}
Instead of giving a confidence interval for \(\sigma ^2\), let's consider using two distributional results for \(M_k\) and \(\hat{\sigma} _n^2\) and apply it to the problem of testing normality, i.e., let \(H_0 \colon X \sim \mathcal{N} (\mu , \sigma ^2)\) for some \(\mu , \sigma ^2\).

\begin{intuition}
	With \autoref{prop:sample-central-moment-consistent}, the idea is that to reject \(H_0\) if \(\vert M_k \vert \) is ``large''.
\end{intuition}

It turns out that considering \(\vert \widetilde{M} _k \vert = \vert M_k / \hat{\sigma} _n^k \vert \) is more appropriate. Hence, we still need to compute the joint of \(M_k\) and \(\hat{\sigma} _n^k\), so we won't miss anything indeed. Anyway, now the problem is the following.

\begin{problem*}
	What is the asymptotic distribution of \(\widetilde{M} _k = M_k / \hat{\sigma} _n^k \)?
\end{problem*}

First observe that since Gaussian is symmetric, \(\mu _k = 0\) (and hence \(\widetilde{\mu} _k = 0\)) for all odd \(k\). It turns out that this property allows us to bypass the joint if we focus on odd \(k\).

\begin{prev}
	\autoref{thm:asymptotic-distribution-sample-central-moment} gives \(\sqrt{n} (M_k - \mu _k) \overset{D}{\to} \mathcal{N} (0, \Var_{}\left[Y^k - k \mu _{k-1} Y\right] ) = \mathcal{N} (0, v_k)\).
\end{prev}
...
\]
Then, by \hyperref[col:Slutsky]{Slutsky's theorem}, \(\sqrt{n} M_k / \hat{\sigma} _n^k\) also \hyperref[def:converge-in-distribution]{converges} to this normal. Let's write this as a corollary for \autoref{thm:asymptotic-distribution-sample-central-moment}.

\begin{corollary}\label{col:asymptotic-distribution-odd-sample-central-moment}
	If \(X \in L^{2k}\) for some odd \(k>2\) such that \(\mu _k = 0\) and \(v_k / \sigma ^k > 0\), then \(\sqrt{n} M_k / \hat{\sigma} _n^k \overset{D}{\to} \mathcal{N} (0, v_k / \sigma ^{2k})\).
\end{corollary}
...
Before we see one concrete example, note the following property when \(\mu _k = 0\) for odd \(k > 2\).
...
\end{eg}
\begin{explanation}
	From \autoref{col:asymptotic-distribution-odd-sample-central-moment} and the symmetry of normal distribution,
	\[
	...
	\]
	where  \(\mu _2 = \sigma ^2\) and \(\Cov_{}\left[Y^3 , Y\right] = \mathbb{E}_{}[Y^4] - \mathbb{E}_{}[Y] \mathbb{E}_{}[Y^3] = \mathbb{E}_{}[Y^4] \). Hence, by plugging \(\Var_{}[Y^3] = \mu _{2 \times 3} = \mu _6\), the variance of the normal is further equal to
	\[
