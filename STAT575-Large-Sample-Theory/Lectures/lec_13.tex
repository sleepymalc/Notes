...
Let's postpone the proof and discuss its implication first. In the case of \(p_n \neq p\), \hyperref[thm:Bahadur-representation]{Bahadur's representation} shows
\[
	\sqrt{n} (\hat{\theta} _{p_n} - \theta _p)
...
\]
This suggests \(\ell _n = p - Z_{\alpha / 2} \sqrt{p (1 - p)} / \sqrt{n} \), and similarly, \(u_n = p + Z_{\alpha / 2} \sqrt{p (1 - p)} / \sqrt{n} \). Another implication is the following. Firstly, consider the case of \(p = 1 / 2\).
...
\begin{eg}
	If \(X\) is Gaussian, then \(\overline{X} \) is a better estimator of \(\mu \) than \(\hat{\theta} _{1 / 2}\).
\end{eg}
\begin{explanation}
	Since
	\[
	...
	\]
	Hence, \(f(\mu ) = 1 / \sigma \sqrt{2\pi } \), i.e.,
	\[
		\operatorname{ARE}(\overline{X} _n, \hat{\theta} _{1 / 2})
...
\begin{eg}
	If \(X\) is Laplace, then \(\hat{\theta} _{1 / 2}\) is a better estimator of \(\mu \) than \(\overline{X} \).
\end{eg}
\begin{explanation}
	Since
	\[
		f(x) = \frac{1}{\sigma \sqrt{2} } e^{- \frac{\vert x - \mu \vert }{\sigma }},
	\]
	Hence, \(f(\mu ) = 1 / \sigma \sqrt{2} \), i.e.,
	\[
		\operatorname{ARE}(\overline{X} _n, \hat{\theta} _{1 / 2})
