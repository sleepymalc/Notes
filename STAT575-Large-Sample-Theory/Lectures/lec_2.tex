...
\begin{theorem}[Scheffé's theorem]\label{thm:Scheffe}
	Let \(X_n \overset{p}{\to } X\). If \(\mathbb{E}_{}[\lVert X_n \rVert ^p] \to \mathbb{E}_{}[\lVert X \rVert ^p] < \infty \), then \(X_n \overset{L^p}{\to } X\). In particular, with \href{https://en.wikipedia.org/wiki/Fatou's_lemma}{Fatou's lemma}, \(\limsup_{n \to \infty} \mathbb{E}_{}\left[\lVert X_n \rVert ^p \right] \leq \mathbb{E}_{}\left[\lVert X \rVert ^p \right] < \infty \) suffices.
\end{theorem}
...
\begin{definition}[Consistent]\label{def:consistent}
	\(\hat{\theta} _n\) is \emph{consistent} of \(\theta \) if \(\hat{\theta} _n \overset{p}{\to } \theta \).
\end{definition}

\begin{definition}[Strongly consistent]\label{def:strongly-consistent}
	\(\hat{\theta} _n\) is \emph{strongly consistent} of \(\theta \) if \(\hat{\theta} _n \overset{\text{a.s.} }{\to } \theta \).
\end{definition}
...
\begin{eg}[Supremum estimation]\label{eg:supremum-estimation}
