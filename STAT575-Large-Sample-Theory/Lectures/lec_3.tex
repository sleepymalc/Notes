\lecture{3}{25 Jan.\ 9:30}{Weak Convergence and }
...
\begin{definition}[Weak convergence]\label{def:weak-convergence}
	\(X_n \) \emph{converges weakly} to \(X\), denoted as \(X_n \overset{\text{w} }{\to } X\), if for all continuous and bounded \(g \colon \mathbb{R} ^d \to \mathbb{R} \), \(\mathbb{E}_{}\left[g(X_n) \right] \to \mathbb{E}_{}\left[g(X) \right]\).
\end{definition}
...
\begin{eg}
	...
	\]
	as \(g\) is continues and bounded on \([0, 1]\), hence Riemann integrable.
\end{eg}
...
\begin{theorem}[Portmanteau theorem]\label{thm:Portmanteau}
	...
	\begin{enumerate}[(a)]
		...
		\item\label{thm:Portmanteau-b} \(\mathbb{E}_{}\left[g(X_n) \right] \to \mathbb{E}_{}\left[g(X) \right] \) for all \(g \colon \mathbb{R} ^d \to \mathbb{R} \), Lipschitz and bounded.
...
Before we prove \hyperref[thm:Portmanteau]{Portmanteau theorem}, we need some result from analysis. Specifically, although we might think of we're working in Euclidean spaces, all our discussion can be extended to metric spaces. Recall some results from metric spaces.
...
\begin{lemma}
	Given a metric space \((S, \rho )\) and let \(A \subseteq S\) be a closed subset. Then there exists \(g_k \colon S \to \mathbb{R} \) decreasing in \(k\) which is bounded and Lipschitz such that \(g_k (x) \searrow \mathbbm{1}_{A}(x) \).
\end{lemma}
\begin{proof}
...
\begin{proof}[Proof of \autoref{thm:Portmanteau}]
	...
	\end{claim}
	\begin{explanation}
		Assuming \(A\) is closed, then
		\[
			\mathbb{P} (X_n \in A)
			= \mathbb{E}_{}\left[\mathbbm{1}_{A} (X_n) \right]
			\leq \mathbb{E}_{}\left[g_k(X_n) \right]
		\]
		This is true for every \(k\) and \(n\) since \(g_k \geq \mathbbm{1}_{A} \). Take the limit as \(n \to \infty \), we have
		\[
	...
	Hence, we now have \autoref{thm:Portmanteau-a} \(\implies \) \autoref{thm:Portmanteau-b} \(\implies \) \autoref{thm:Portmanteau-d} \(\iff \) \autoref{thm:Portmanteau-c}.
\end{proof}