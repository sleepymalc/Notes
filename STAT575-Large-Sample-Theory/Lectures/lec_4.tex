...
Before finishing the proof of \hyperref[thm:Portmanteau]{Portmanteau theorem}, we need one additional tool.
...
\begin{proof}[Proof of \autoref{thm:Portmanteau} (cont.)]\label{pf:thm:Portmanteau-cont}
	...
	\begin{explanation}
		...
		\[
			\begin{split}
				\mathbb{P} (X \in A^o)
				 & \leq \liminf_{n \to \infty} \mathbb{P} _{n}(X_n \in A^o)
				\leq \liminf_{n \to \infty} \mathbb{P} _{n}(X_n \in A)      \\
				 & \leq \limsup_{n \to \infty} \mathbb{P} _{n}(X_n \in A)
				\leq \limsup_{n \to \infty} \mathbb{P} _{n}(X_n \in \overline{A} )
		...
		\[
			...
			\leq \lim_{n \to \infty} \mathbb{P} _{n}(X_n \in A)
			\leq \limsup_{n \to \infty} \mathbb{P} _{n}(X_n \in A)
		\]
		and \(\mathbb{P} (X \in A^o) \leq \mathbb{P} (X \in A) \leq \mathbb{P} (X \in \overline{A} ) \), \(\mathbb{P} (X \in A) = \lim_{n \to \infty} \mathbb{P} _{n}(X_n \in A)\).
	\end{explanation}
	...
	\end{claim}
	\begin{explanation}
		For every \(g \colon \mathbb{R} ^d \to \mathbb{R} \) bounded and continuous, we want to show \(\mathbb{E}_{n}\left[g(X_n) \right] \to \mathbb{E}_{}\left[g(X) \right] \). Suppose \(g \geq 0\),\footnote{Otherwise, we consider \(g = g^+ - g^-\) where \(g^+ = \max (g, 0)\) and \(g^- = \max (-g, 0)\), and everything follows.} and let \(K \geq g(x)\) for every \(x \in \mathbb{R} ^d\) (which exists since \(g\) is bounded), then
		\[
			\mathbb{E}_{n}\left[g(X_n) \right]
			= \int_{0}^{K} \mathbb{P} _{n}(g(X_n) > t) \,\mathrm{d}t , \quad
		...
		\[
			\mathbb{P} _{n}(g(X_n) > t) \to \mathbb{P} (g(X) > t).
		\]
		Observe that \(\mathbb{P} _{n}(g(X_n) > t) = \mathbb{P} _{n}(X_n \in \{ g > t \} )\) and \(\mathbb{P} (g(X) > t) = \mathbb{P} (X \in \{ g > t \} )\), so from \autoref{thm:Portmanteau-e} with \(A \coloneqq \{ g > t \} \), it suffices to show \(\mathbb{P} (X \in \partial \{ g > t \} ) = 0\) for almost all \(t\). Firstly,
		\[
...
\begin{proof}
	...
	\[
		\limsup_{n \to \infty} \mathbb{P} _{n}(h(X_n) \in A)
		\leq \mathbb{P} (h(X) \in A).
	\]
	Since \(\limsup_{n \to \infty} \mathbb{P} _{n}(h(X_n) \in A) = \limsup_{n \to \infty} \mathbb{P} _{n}(X_n \in \{ h \in A \} )\), implying
	\[
		\limsup_{n \to \infty} \mathbb{P} _{n}(h(X_n) \in A)
		\leq \limsup_{n \to \infty} \mathbb{P} _{n}(X_n \in \overline{\{ h \in A \}} )
...
\begin{theorem}[Converging together]\label{thm:converging-together}
	Let \(X_n \overset{\text{w} }{\to } X\), and if \(Y_n\) on the same probability space as \(X_n\) such that \(\lVert X_n - Y_n \rVert \overset{p}{\to } 0\), i.e., for all \(\epsilon > 0\), \(\mathbb{P} _{n}(\lVert X_n - Y_n \rVert > \epsilon ) \to 0\) as \(n \to \infty\). Then, \(Y_n \overset{\text{w} }{\to } X\).
\end{theorem}

The following corollary draws connections between \hyperref[def:converge-weakly]{weak convergence} and \hyperref[def:converge-in-probability]{convergence in probability}.
...
\begin{proof}
	...
	\[
		...
		\leq \liminf_{n \to \infty} \mathbb{P} _{n}(Y_n \in B(c, \epsilon )),
	\]
	implying \(\mathbb{P} _{n}(Y_n \in B(c, \epsilon )) \to 1\), i.e., \(\mathbb{P} _{n}(\lVert Y_n - c \rVert < \epsilon ) \to 1\).
\end{proof}

\begin{remark}
	\hyperref[def:converge-weakly]{Weak convergence} doesn't give \hyperref[def:converge-in-probability]{convergence in probability} even if \((\Omega _n, \mathscr{F} _n, \mathbb{P} _n) = (\Omega , \mathscr{F} , \mathbb{P} )\).
\end{remark}

\begin{eg}
	Let \(X \sim \mathcal{N} (0, 1)\), \(Y_n = - X\) for all \(n \geq 1\). Then, \(Y_n \overset{\text{w} }{\to} X\), but clearly not \hyperref[def:converge-in-probability]{in probability}.
\end{eg}