\lecture{5}{1 Feb.\ 9:30}{Converge in Distribution and Weak Convergence}
...
We can now apply \autoref{thm:converging-together} to prove something similar as we have seen before in the case of \hyperref[def:convergence-in-probability]{convergence in probability}.
...
\begin{proof}
	...
	\[
		...
		= \lVert Y_n - c \rVert ,
	\]
	which goes to \(0\) \hyperref[def:convergence-in-probability]{in probability} as we wish. Combining both gives the result.
\end{proof}
...
\]
same for \(F_X\). There are several remarks to be made here.
...
\begin{note}
	Since \(F_{X_n}\) and \(F_X\) is only defined for random vectors (ordering might not be available).
\end{note}
...
The virtue of \hyperref[def:convergence-in-distribution]{convergence in distribution} is that it's actually just a renamed version of \hyperref[def:weak-convergence]{weak convergence} in \(\mathbb{R} ^d\).
