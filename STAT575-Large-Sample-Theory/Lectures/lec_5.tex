...
\begin{proof}[Proof of \autoref{thm:converging-together}]
	...
	\[
		...
		\end{split}
	\]
	As \(n \to \infty \), \(\limsup_{n \to \infty} \left\vert \mathbb{E}_{}\left[g(Y_n) \right] - \mathbb{E}_{}\left[g(X) \right]  \right\vert \leq K \epsilon\) for all \(\epsilon > 0\), by letting \(\epsilon \to 0\), we're done.
\end{proof}

We can now prove something similar as we have seen before for \hyperref[def:converge-in-probability]{convergence in probability}.
...
\end{theorem}
\begin{proof}
	Firstly, we show that \((X_n , c) \overset{\text{w} }{\to } (X, c)\). Indeed, since for every continuous and bounded \(g \colon \mathbb{R} ^{d+m} \to \mathbb{R} \), from \(X_n \overset{\text{w} }{\to } X\) with \(g(\cdot, c)\) being continuous and bounded, \(\mathbb{E}_{}\left[g(X_n, c) \right] \to \mathbb{E}_{}\left[g(X, c) \right]\).
...
\]
same for \(F_X\). So this reduces to the form we're familiar with, i.e., \(\mathbb{P} (X_n \in A)\) for some \(A\). Let's make some remarks for this new notion of convergence.
...
\begin{remark}[De Moivre central limit theorem]
...
\begin{proof}
	...
	\]
	As \(t \pm \epsilon \) are in \(C_X\), \(X_n \overset{D}{\to } X\) implies \(\mathbb{P} (X_n \leq t + \epsilon ) \to \mathbb{P} (X \leq t + \epsilon )\). The same holds for \(t - \epsilon \), hence
	\[
		f_n(t) = \mathbb{P} (X_n = t)
		= \mathbb{P} (X_n \leq t + \epsilon ) - \mathbb{P} (X_n \leq t - \epsilon )
		...
		= f(t).
	\]
	Since this holds for every \(t\in \mathbb{Z} \), we're done.
\end{proof}
...
\begin{remark}[Polya's theorem]
	If \(X_n \overset{D}{\to } X\) and \(X\) is continuous, then \(F_{X_n}\) converges to \(F_X\) not only point-wise, but uniformly. Specifically, if \(F_X\) is continuous, \(X_n \overset{D}{\to } X\) is equivalent as
	\[
