...
Before we finish the proof of \autoref{thm:weak-convergence-is-convergence-in-distribution}, we need recall one important characterization of \(\liminf\).
...
In our context, we might expect something like ``if \(X_n \overset{p}{\to } X \), then \((X_n)\) is \hyperref[def:bounded-in-probability]{bounded in probability}.'' In fact, we have the following ``stronger'' result where we only require \hyperref[def:convergence-in-distribution]{convergence in distribution}.
...
\begin{eg}
	...
	\[
		\frac{\sum_{i=1}^{n} X_i - n p}{\sqrt{n p (1 - p)} }
		= \sqrt{\frac{n}{p (1 - p)}} \left( \frac{\sum_{i=1}^{n} X_i}{n} - p \right)
...
\begin{theorem}[Delta method]\label{thm:delta-method}
	Let \((T_n)\) be \(d\)-dimensional random vectors with a deterministic sequence \(b_n \to \infty \) such that \(b_n (T_n - \theta ) \overset{D}{\to } Y\). If \(g \colon \mathbb{R} ^d \to \mathbb{R} ^m\) is differentiable at \(\theta \), then
	\[
		\lVert b_n (g(T_n) - g(\theta )) - \nabla g(\theta ) b_n(T_n - \theta ) \rVert \overset{p}{\to } 0.
	\]
\end{theorem}
\begin{proof}
	Since \(g\) is differentiable at \(\theta \), as \(x \to \theta \),
	\[
	...
	\]
	We want to show that the right-hand sides goes to \(0\) \hyperref[def:convergence-in-probability]{in probability}. Observe that it's enough to show \(\lVert h(T_n) \rVert = o_p(1)\) and \(\lVert b_n (T_n - \theta ) \rVert \in O_p(1)\). Indeed:
	\begin{itemize}
		\item \(\lVert b_n (T_n - \theta ) \rVert \in O_p(1)\): Since \(b_n(T_n - \theta ) \overset{D}{\to } Y\), with \hyperref[thm:continuous-mapping]{continuous mapping theorem} and the fact that \(\lVert \cdot \rVert \) is continuous, \(\lVert b_n (T_n - \theta ) \rVert \overset{D}{\to } \lVert Y \rVert \), so \(\lVert b_n (T_n - \theta ) \rVert \in O_p(1)\) by \autoref{prop:convergence-in-distirbution-bounded-in-probability}.
		\item \(\lVert h(T_n) \rVert = o_p(1)\): since \(b_n \to \infty \),
		      \[
			      \lVert T_n - \theta \rVert
			      = \frac{1}{b_n} \lVert b_n (T_n - \theta ) \rVert
			      \overset{p}{\to } 0,
		      \]
		      i.e., \(T_n \overset{p}{\to } \theta \). This implies \(h(T_n) \overset{p}{\to } h(\theta )\) again by \hyperref[thm:continuous-mapping]{continuous mapping theorem} with \(h\) being continuous at \(\theta \). This further implies \(\lVert h(T_n) \rVert \overset{p}{\to } 0\) as we desired.\footnote{This involves \hyperref[thm:continuous-mapping]{continuous mapping theorem} and \autoref{col:weak-probability-constant} since \(h(\theta ) = 0\), a constant (so does its norm).}
	\end{itemize}
