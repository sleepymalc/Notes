...
\begin{remark}
	If \(X_n \overset{D}{\to } X\), from \hyperref[thm:Skorohod-representation]{Skorohod's representation theorem}, we can obtain \(Y_n \overset{\text{a.s.} }{\to } Y\) on \((\widetilde{\Omega} , \widetilde{\mathscr{F}} , \widetilde{\mathbb{P}} )\) such that \(X_n \overset{D}{=} Y_n\) and \(X \overset{D}{=} Y\). Then by the \href{https://en.wikipedia.org/wiki/Dominated_convergence_theorem}{bounded convergence theorem}, for any bounded and continuous \(g\),
	\[
...
\subsection{Proving Distributional Convergence}
We often want to prove \(X_n \overset{D}{\to } X\), which is not efficient if we start from the \hyperref[def:converge-in-distribution]{definition}. To get some intuition for potential proof strategies, consider a deterministic sequence \((x_n)\) in a metric space \((S, \rho )\).
