...
We often want to prove \(X_n \overset{D}{\to } X\), which is not efficient if we start from definition. To get some intuition for potential proof strategies, consider a deterministic sequence \((x_n)\) in a metric space \((S, \rho )\).
