...
\begin{proposition}\label{prop:lec7}
	If \(X_n \overset{\text{w} }{\to } X\) and \(\mathbb{P} (X \in C_g) = 1\) where \(g \colon \mathbb{R} ^d \to \mathbb{R} \) such that \((g(X_n))\) is uniformly integrable, then \(\mathbb{E}_{}[g(X_n)] \to \mathbb{E}_{}[g(X)] \).
\end{proposition}
\begin{proof}
