...
\begin{proof}
	...
	\begin{align*}
		...
		\shortintertext{by replacing \(e^{-(x-y)^2 / 2\sigma ^2}\) from \autoref{eq:thm:characteristic-function-uniqueness} with \(t = (x - y) / \sigma \),}
		 ...
		 & = \frac{1}{2\pi } \iint e^{-i x u - \sigma ^2 u^2 / 2} \underbrace{\int e^{i y u} F_X(\mathrm{d} y)}_{\phi _X(u)} \,\mathrm{d} u ,
	\end{align*}
	where we interchange the order of integrals with \href{https://en.wikipedia.org/wiki/Fubini's_theorem}{Fubini's theorem} when integrands are absolute integrable. This implies that \(F_{X + \sigma Z} (\mathrm{d} x)\) can be written in terms of \(\phi _X\) where with no other dependencies, hence we're done.
\end{proof}
...
\begin{theorem}[Lévy-Cramer continuity theorem]\label{thm:Levy-Cramer-continuity}
