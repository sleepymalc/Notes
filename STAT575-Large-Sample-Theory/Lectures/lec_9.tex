...
\begin{proposition}\label{prop:characteristic-function-uniformly-continuous}
	For any random vector \(X\), \(\phi _X\) is uniformly continuous, i.e., as \(h \to 0\),
	\[
...
\begin{proof}
	...
	\]
	which goes to \(0\) as \(h \to 0\) from the \href{https://en.wikipedia.org/wiki/Dominated_convergence_theorem}{bounded convergence theorem}.
\end{proof}
...
\begin{remark}
	\autoref{thm:characteristic-function-derivative} implies \(\sup _t \vert \phi _X^{(p)} (t) \vert \leq \mathbb{E}_{}[\vert X \vert ^p]\).
\end{remark}