...
\begin{intuition}[Main bound]\label{int:main-bound}
	Let \(\mathscr{F} \) be a boolean function class, then since \(\sup _{f\in\mathscr{F} } \leq 1\),
	\[
	...
	\]
	More generally, if we have some \(F \geq f\) (called \hyperref[def:envelope]{envelope}) for all \(f\in \mathscr{F} \) such that \(\mathbb{P} F^2 < \infty \), this holds.
\end{intuition}
