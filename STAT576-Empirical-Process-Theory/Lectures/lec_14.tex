...
\begin{proof}
	...
	\]
	We see that \(N \leq (\log N)^d\). To further bound \(N\), consider\footnote{We see that this works as long as we make the exponent \(a\) of \(\log N^a\) \(< 1 / d\).}
	\[
...
\todo[inline]{not sure how does \(R_n(\mathscr{F} ) \leq C \sqrt{\VC(\mathscr{F} ) / n} \) follow.}
