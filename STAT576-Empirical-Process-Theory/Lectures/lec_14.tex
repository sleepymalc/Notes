...
\begin{prev}
	...
	\[
		...
		= R_n(\mathscr{F} )
	\]
	where the inner expectation is just the \hyperref[def:Rademacher-width]{Rademacher width} \(R_n (\{ f(x_1) / \sqrt{n} , \dots , f(x_n) / \sqrt{n} \}_{f\in \mathscr{F} } )\). Let \(X_f = \langle \epsilon , f \rangle / \sqrt{n} \), then \(\{ X_f \} _{f\in \mathscr{F} }\) is \hyperref[def:sub-Gaussian-process]{sub-Gaussian} w.r.t.\ \(L_2(\mathbb{P} _n)\).\footnote{Recall that \(L_2^2(\mathbb{P} _n)(f, g) = \frac{1}{n}\sum_{i=1}^{n} \big(f(X_i) - g(X_i)\big)^2\), compared to \(L_2(\mathbb{P} )(f, g) = \int \big(f(x) - g(x)\big)^2 \,\mathrm{d} \mathbb{P} \).}
\end{prev}
...
\begin{proof}
	...
	\begin{align*}
		\mathbb{E}_{}\left[\sup _{f\in \mathscr{F} } \sqrt{n} (\mathbb{P} _n f - \mathbb{P} f) \right]
		 & \leq 2 \mathbb{E}_{x}\left[\mathbb{E}_{\epsilon }\left[\sup _{f\in \mathscr{F} } \frac{1}{\sqrt{n} } \sum_{i=1}^{n} \epsilon _i f(x_i) \right]  \right] \text{(\(= 2R_n (\mathscr{F} )\))}                                              \tag*{\hyperref[lma:symmetrization]{symmetrization}}                                                             \\
