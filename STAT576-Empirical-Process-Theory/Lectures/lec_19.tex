...
\begin{enumerate}[(a)]
	...
	\item What's the ``rate of convergence''? E.g., the rate for the mean is \(O(1 / \sqrt{n} )\).
	\item What is the limiting (asymptotic) distribution of \(\hat{\theta} - \theta _0\)?
\end{enumerate}

\subsection{Running Example}
In the remaining class, we will consider the \hyperref[prb:mode-estimation]{sample mode estimation} as our running example.

\begin{problem}[Mode estimation]\label{prb:mode-estimation}
...
\begin{remark}[Unique optimal]
	Notice that in the \hyperref[prb:mode-estimation]{mode estimation} problem, we can check that
	\begin{itemize}
...
Consistency is the easiest task among others, and we do this without making it into its own section. To check it, recall the ``basic inequality'' step, i.e.,
\[
...
In the example of \hyperref[prb:mode-estimation]{mode estimation} example, denote \(m_\theta (x) = \mathbbm{1}_{\theta -1 \leq x \leq \theta +1} \), and \(\mathscr{F} = \{ m_\theta \colon \theta \in \mathbb{R} \} \), then \(M_n(\theta ) = \mathbb{P} _n m_\theta \) and \(M(\theta ) = \mathbb{P} m_\theta \). We have
...
\begin{problem*}
	Does \(\hat{\theta} \overset{p}{\to} \theta _0\), i.e., does \(d(\hat{\theta} , \theta _0) \to 0\)?
\end{problem*}
\begin{answer}
