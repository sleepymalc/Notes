\chapter{Bounding Supremum of Empirical Process}
\lecture{2}{23 Aug. 9:00}{Concentration Inequalities and the MGF Trick}
Most of this course will focus on bounding suprema of the \hyperref[def:empirical-process]{empirical process}. Let's first remind what the goal is:
\begin{prev}
	Given a domain \(\chi \), probability measure \(\mathbb{P} \) on \(\chi \), data \(X_1, \dots , X_n \overset{\text{i.i.d.} }{\sim } \mathbb{P} \), and a function class \(\mathscr{F}\ni f \colon \chi \to \mathbb{R}  \). We would like to bound (non-asymptotically)
	\[
...
For us, the gold standard for concentration would be the Gaussian distribution. The property of the Gaussian distribution we are interested in now is its rapid tail decay. This is given in \autoref{lma:Gaussian-tail-bound}.
...
\subsection{MGF Trick}
The most basic tool to bound tail probabilities is the \hyperref[lma:Markov-inequality]{Markov's inequality}.
...
\end{problem*}
\begin{answer}
	Yes, if we assume more moments exist. If all moments exist and in particular the MGF (moment generating function) exists, like for the Gaussian, then we can expect faster tail decay.
\end{answer}

For example, if we assume second moment exists, then we can get a \(O(1 / t^2)\) tail decay by \hyperref[lma:Chebyshev-inequality]{Chebyshev inequality}.
...
In the same vein, we can now assume the MGF exists and apply \hyperref[lma:Markov-inequality]{Markov's inequality}.
...
\begin{proof}
	...
	\]
	where the last inequality is obtained by minimizing the quadratic function \(\lambda ^{2} \sigma ^{2} / 2 - \lambda t\) whose minimizer is \(\lambda ^{\ast} = t / \sigma ^{2} \). The same bound holds for the left tail and a union bound gives the two-sided version.
\end{proof}
...
We can now immediately prove the \hyperref[thm:Hoeffding-inequality]{Hoeffding's inequality}.
...
\begin{theorem}[Hoeffding's inequality for sub-gaussian random variables]\label{thm:Hoeffding-inequality}
	...
	\]
	for all \(t \geq 0\).\footnote{One-sided version holds without the factor \(2\).}
\end{theorem}
\begin{proof}
