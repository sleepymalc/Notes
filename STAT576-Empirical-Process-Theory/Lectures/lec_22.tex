...
\begin{proof}
	...
	\]
	where \(\mathscr{F} = \{ f_{jk} \colon 0 \leq j, k \leq p\} \) with \(f_{jk} = X_j X_k - \mathbb{E}_{}\left[X_j X_k \right] \). Now, \(F\) is clearly an \hyperref[def:envelope]{envelope} with \(\lVert F \rVert _{L_2(\mathbb{P} )} < \infty \), and by defining \hyperref[def:eps-bracket]{\(\epsilon\)-brackets} to be \(\{ [f_{j,k} \pm \epsilon / 2 ]\} \), we have
	\[
