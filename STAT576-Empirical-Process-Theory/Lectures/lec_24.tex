...
Now, we consider the \hyperref[prb:fixed-design-non-parametric-LS]{fixed design} case with well specified case, i.e., given \(x_1, \dots , x_n \in \chi \), let \(y_i = f^{\ast} (x_i) + \epsilon _i\) and assume \(\epsilon _1, \dots , \epsilon _n \overset{\text{i.i.d.} }{\sim } \mathcal{N} (0, 1) \).\footnote{This can be generalized to any \(\Subg(\sigma )\).} We further assume that \(f^{\ast} \in \mathscr{F} \).

Constrained least sqaures where
\[
