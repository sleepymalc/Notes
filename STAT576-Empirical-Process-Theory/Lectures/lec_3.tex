...
\begin{eg}
	...
	This suggests that \(\sum_{i} X_i = O(\sqrt{n})\) whereas we know that in this case that the distribution of \(\sum_{i} X_i\) is close to the \(\operatorname{Poisson}(\lambda)\) and thus should be \(O(1)\).
