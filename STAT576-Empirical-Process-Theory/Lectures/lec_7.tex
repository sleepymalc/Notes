...
\begin{prev}
	If \(X_i \sim \mathop{\mathrm{Subg}}(\sigma ^2) \) independent, \(\sum_{i} a_i X_i \sim \mathop{\mathrm{Subg}}((\sum_{i} a_i^2) \sigma ^2) \) from \autoref{lma:sub-gaussian-add}.
\end{prev}

\begin{remark}
	Let \(a_i = 1/n\), we see that \(\mathbb{E}_{}\left[\mathbbm{1}_{X \leq \theta _j}\right] - \frac{1}{n} \sum_{i=1}^{n} \mathbbm{1}_{x_i \leq \theta _j} \in \mathop{\mathrm{Subg}}(1 / n) \).\footnote{Since it's bounded between \(0\) and \(1\).}
\end{remark}

Finally, recall what we have proved in the homework.

\begin{lemma}\label{lma:lec7}
	Let \(X_1, \dots , X_n \sim \mathop{\mathrm{Subg}}(\sigma ^{2} ) \),\footnote{Not necessary independent.} then \(\mathbb{E}_{}\left[\max _i X_i \right] \leq \sqrt{2 \sigma ^{2} \log n}\).
\end{lemma}

Using \autoref{lma:lec7}, since we have \((N+1)\) random variables with variance factor \(1 / n\), by choosing \(N+1 \coloneqq n\),\footnote{Recall that \(n\) is the sample size, so we can choose the corresponding \(n\) to meet the requirement.} \autoref{eq:bracketing} can be further bounded by
\[
...
\begin{definition}[Rademacher complexity]\label{def:Rademacher-complexity}
	...
	\[
		R_n(\mathscr{F} ) \coloneqq 2 \mathbb{E}_{\epsilon _i, X_i \sim \mathbb{P} }\left[\sup _{f\in \mathscr{F} }\left\vert \frac{1}{n}\sum_{i=1}^{n} \epsilon _i f(X_i) \right\vert  \right] .
	\]
\end{definition}
