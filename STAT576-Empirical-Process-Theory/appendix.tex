...
\begin{proof}
	...
	\begin{explanation}
		...
		\]
	\end{explanation}
	In all, from the \hyperref[clm:metric-entropy-a]{first claim}, \(N(C(\alpha )\epsilon , \mathcal{S} _\alpha , d ) \leq \vert I \vert \leq \exp (C(\alpha )\epsilon ^{-1 / \alpha })\) as desired, hence \(M(\mathcal{S} _\alpha , d, \epsilon ) \leq \exp(c_1 \epsilon ^{-1 / \alpha })\) for some \(c_1\). As for the lower-bound, consider the following.\todo{Check}
	\begin{claim}
